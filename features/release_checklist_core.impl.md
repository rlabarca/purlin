...
*   **Removal rationale (`purlin.mark_release_complete`):** This step assumed per-version release specification files (e.g., `release_v0.5.md`) marked `[Complete]` at release time. This project does not use per-version release files. The only release-related feature files are specs for the release checklist tool itself, which follow the standard CDD feature lifecycle and are not a release-time PM action. The step had no valid target and was retired.
