...
Token mappings MUST validate against the declared design anchor's token definitions. For Purlin tools, this is `features/design_visual_standards.md` Section 2.2. Consumer projects validate against their own `design_*.md` anchor. Invalid mappings are flagged as PM action items by `/pl-status`.
...
*   Unprocessed artifacts (reference exists but no Token Map) are flagged as HIGH-priority PM action items by `/pl-status`.
...
*   **Advisory gate:** `/pl-status` generates a LOW-priority PM action item when a feature is in Engineer TODO state with `Figma Status: Design`. This is advisory, not blocking -- the human decides whether to wait for design completion.
