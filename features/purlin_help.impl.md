# Implementation Notes: Skill -- purlin:help
...
The skill reads the role-appropriate command table from `references/{role}_commands.md` and prints the correct variant based on the current branch (main, collaboration, or isolated). This is the same logic used by the Startup Print Sequence and `purlin:resume`.

### Role Detection
Uses the same 3-tier fallback as `purlin:resume`: explicit argument, system prompt inference, then user prompt.

### Audit Finding -- 2026-03-23

**[DISCOVERY] [ACKNOWLEDGED]** Skill contradicts spec on --help execution
**Source:** purlin:spec-code-audit --deep (H14)
**Severity:** HIGH
