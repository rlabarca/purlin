# Feature: purlin:remote Branch Collaboration
...
The `purlin:remote` skill is the single entry point for branch collaboration operations. It provides four subcommands — `push`, `pull`, `add`, and `branch` — that manage remote configuration, branch lifecycle, and synchronization between machines. Push and pull resolve the collaboration branch from `.purlin/runtime/active_branch` (defaulting to `main` when absent), enforce safety preconditions, and operate within the sync-state model defined in `policy_branch_collab.md`. Branch manages the collaboration branch lifecycle (create, join, leave, list). Add handles remote configuration with guided setup, SSH key management, and connectivity verification.
...
- **Legacy agent routing:** Legacy agents use `purlin:remote-push`, `purlin:remote-pull`, `purlin:remote-add` instead.
...
|------------|-------------------|
| `add` success | `purlin:remote branch create <name>` or `purlin:remote push` |
| `branch create` success | `purlin:remote push` and `purlin:remote pull` to sync |
| `branch join` success | `purlin:remote push` and `purlin:remote pull` to sync |
| `branch leave` success | `purlin:remote branch join <name>` to rejoin |
| `branch list` (no branches) | `purlin:remote branch create <name>` to start one |
| `push` / `pull` — no remote | `purlin:remote add` to configure a remote |
| `push` / `pull` — no active branch, wrong branch | `purlin:remote branch create <name>` or switch to main |
| `push` — BEHIND/DIVERGED | `purlin:remote pull` before pushing |
| `pull` — AHEAD | `purlin:remote push` when ready |
...
- **No active branch (direct mode):** Abort: `"No active collaboration branch. On main, purlin:remote <push|pull> operates on main directly. Current branch: <current>. Switch to main or create a collaboration branch with purlin:remote branch create <name>."`
...
If no remotes exist: `"No git remote configured. Run purlin:remote add to set up a remote first."` Exit 1.
...
4. **BEHIND:** `"Local <branch> is BEHIND <remote>/<branch> by M commits. Run purlin:remote pull before pushing."` Exit 1.
5. **DIVERGED:** Print incoming commits from remote. Instruct to run `purlin:remote pull`. Exit 1.
...
- **Explicit-branch mode** (`purlin:remote pull RC0.8.0`): Use the argument as `<branch>` directly. Skip Branch Resolution and Branch Guard. Execute Remote Guard (2.7) → Working Tree Check (2.8) → Config Reading (2.4). Runs from any branch (typically `main`).
...
4. **AHEAD:** `"Local <branch> is AHEAD by N commits. Nothing to pull — run purlin:remote push when ready."` Exit 0.
5. **BEHIND:** `git merge --ff-only <remote>/<branch>`. Report `"Fast-forwarded local <branch> by M commits from <remote>/<branch>."` On ff-failure (race condition): `"Fast-forward failed — re-run purlin:remote pull."` Exit 1.
...
```
purlin:remote add [<url>] [--name <remote-name>]
```
...
```
purlin:remote add — Configure a git remote for this project

This connects your project to a remote repository so you can
push, pull, and collaborate on branches with purlin:remote.
...
- Name does NOT match and is the **only** remote: Prompt: `"Remote <name> is not the default (origin). Should purlin:remote push and purlin:remote pull use <name> as the default remote? [Y/n]"` If yes, write `branch_collab.remote` to `.purlin/config.json`.
...
```
purlin:remote branch create <name>   — Create and switch to a collaboration branch
purlin:remote branch join <name>     — Switch to an existing remote branch
purlin:remote branch leave           — Return to main and clear active branch
purlin:remote branch list            — List available collaboration branches
purlin:remote branch                 — (no sub-subcommand) Print branch subcommand usage
```
...
If no collaboration branches exist: `"No collaboration branches on <remote>. Use purlin:remote branch create <name> to start one."`
...
#### Scenario: branch create Creates And Pushes New Branch
    ...
    When purlin:remote branch create feature/collab is invoked
...
#### Scenario: branch create Rejects When No Remote
    ...
    When purlin:remote branch create feature/collab is invoked
    Then the command prints "No git remote configured. Run purlin:remote add to set up a remote first."
    And exits with code 1
...
#### Scenario: branch join Switches To Existing Remote Branch
    ...
    When purlin:remote branch join feature/collab is invoked
...
#### Scenario: branch join Rejects Nonexistent Remote Branch
    ...
    When purlin:remote branch join feature/nonexistent is invoked
    Then the command prints that the branch does not exist on the remote
    And suggests purlin:remote branch create or purlin:remote branch list
    And exits with code 1
...
#### Scenario: branch leave Returns To Main
    ...
    When purlin:remote branch leave is invoked
...
#### Scenario: branch list Shows Remote Branches
    ...
    And .purlin/runtime/active_branch contains "feature/collab"
    When purlin:remote branch list is invoked
...
#### Scenario: branch list Shows Empty Message When No Branches
    ...
    And suggests purlin:remote branch create
...
#### Scenario: push Resolves To Main When No Active Branch
    ...
    When purlin:remote push is invoked
...
#### Scenario: push Blocked When Local Is BEHIND Remote
    ...
    When purlin:remote push is invoked
    Then the command prints "Local feature/auth is BEHIND" and instructs to run purlin:remote pull
    And exits with code 1
...
#### Scenario: push Blocked When Local Is DIVERGED
    ...
    And instructs to run purlin:remote pull
    And exits with code 1
...
#### Scenario: pull Resolves To Main When No Active Branch
    ...
    When purlin:remote pull is invoked
    Then the collaboration branch resolves to main
...
#### Scenario: pull Generates Digest After Successful Merge
    ...
    Given the current branch is feature/auth with an active branch "feature/auth"
    And origin/feature/auth has 3 commits not in local feature/auth
    When purlin:remote pull is invoked and the fast-forward succeeds
...
#### Scenario: pull Explicit Branch Skips Branch Guard
    ...
    When purlin:remote pull RC0.8.0 is invoked
...
#### Scenario: add Prints Help Banner When No Args And No Remote
    ...
    Given no git remotes are configured
    When purlin:remote add is invoked with no arguments
...
#### Scenario: add Guides Setup When No Remote Exists
    ...
    When purlin:remote add completes the guided setup
...
#### Scenario: add Skips Banner When URL Argument Provided
    ...
    Given no git remotes are configured
    When purlin:remote add git@github.com:user/repo.git is invoked
...
#### Scenario: add Accepts Both URL And Name Arguments
    ...
    Given no git remotes are configured
    When purlin:remote add git@github.com:user/repo.git --name upstream is invoked
...
#### Scenario: add Changes URL When Named Remote Exists And URL Provided
    ...
    When purlin:remote add git@github.com:user/new-repo.git --name origin is invoked
...
#### Scenario: add Adds New Remote When Named Remote Does Not Exist
    ...
    Given a remote "origin" is configured
    When purlin:remote add git@github.com:user/fork.git --name upstream is invoked
...
#### Scenario: add Restores Old URL On Set-Url Failure
    ...
    Given a remote "origin" is configured with URL "git@github.com:user/old-repo.git"
    When purlin:remote add git@bad-host:user/repo.git --name origin is invoked
...
#### Scenario: add Normalizes Browser URL to SSH
    ...
    Given no git remotes are configured
    When purlin:remote add https://bitbucket.org/team/repo/src/main/ is invoked
...
#### Scenario: add Sets Up SSH Key On Auth Failure
    ...
    When purlin:remote add git@bitbucket.org:team/repo.git is invoked
...
#### Scenario: add Does Not Require Branch Guard
    ...
    And the current branch is main
    When purlin:remote add is invoked
...
#### Scenario: add Does Not Push Or Pull
    ...
    Given no git remotes are configured
    When purlin:remote add is invoked and the remote is successfully added
...
#### Scenario: add Skips Config Sync When Name Is Origin
    ...
    Given no git remotes are configured
    When purlin:remote add git@github.com:user/repo.git --name origin is invoked
    And connectivity verification succeeds
...
#### Scenario: Invalid Subcommand Prints Usage
    ...
    Given purlin:remote is invoked with argument "foo"
...
#### Scenario: No Subcommand Prints Usage
    ...
    Given purlin:remote is invoked with no arguments
    When the command parses the subcommand
    Then the command prints usage with the four valid subcommands (push, pull, add, branch)
    And exits without performing any git operations
...
#### Scenario: Config Reads branch_collab Before remote_collab
    ...
    When any purlin:remote subcommand resolves the remote name
...
#### Scenario: add Success Suggests Next Steps
    ...
    Given no git remotes are configured
    When purlin:remote add completes successfully
    Then the success output includes "purlin:remote branch create" as a suggested next step
    And the success output includes "purlin:remote push" as a suggested next step

#### Scenario: push No Remote Suggests add

    Given no git remotes are configured
    When purlin:remote push is invoked
    Then the error message includes "purlin:remote add"

#### Scenario: branch create Success Suggests push and pull

    Given purlin:remote branch create completes successfully
    When the success output is displayed
    Then it includes "purlin:remote push" and "purlin:remote pull" as next steps
    And it includes "purlin:remote branch leave" to return to main

#### Scenario: branch list Empty Suggests create

    Given no collaboration branches exist on the remote
    When purlin:remote branch list is invoked
    Then the output includes "purlin:remote branch create <name>" as a suggestion
