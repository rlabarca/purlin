...
*   **Typography Alignment (2026-02-21):** Aligned CDD and Software Map with `design_visual_standards.md` Section 2.3. Added `--font-display` and `--font-body` CSS custom properties to both `:root` and `[data-theme='light']` blocks. Updated CDN weights to Montserrat 800,900 and Inter 400,500,700. Page title uses `var(--font-display)` at weight 900 with `letter-spacing:0.12em;text-transform:uppercase` (wide tracking matching SVG logo treatment). Section headers (h2) use `var(--font-body)` at weight 700, uppercase, `letter-spacing:0.1em`. Sub-labels (h3) and table headers use `var(--font-body)` at weight 700, uppercase, `letter-spacing:0.1em`. All hardcoded `'Montserrat'` and `'Inter'` font-family values replaced with CSS custom property references.
*   **[CLARIFICATION]** Title font-size kept at 14px (CDD) / 14px (Software Map) despite design spec guideline of 32-40px. The compact monitoring dashboard layout requires a smaller title. The weight (900), letter-spacing (0.12em), text-transform (uppercase), and font-family (var(--font-display)) match the spec. (Severity: INFO)
