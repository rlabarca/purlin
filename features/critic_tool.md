...
#### Scenario: Fixture Repo Not Found Produces Builder Warning
    Given a feature spec contains a fixture tag section with declared tags
    And no fixture repo exists at the convention path `.purlin/runtime/fixture-repo`
    And the feature has no `> Test Fixtures:` metadata override
    ...
    And the description instructs to run the setup script to create the fixture repo
    And individual missing-tag checks are skipped (no repo to check against)

#### Scenario: Convention Path Fixture Repo Used Without Configuration
    Given a feature spec contains a fixture tag section with declared tags
    And the feature has no `> Test Fixtures:` metadata
    And `.purlin/config.json` has no `fixture_repo_url` key
    And a bare git repo exists at `.purlin/runtime/fixture-repo`
    When the Critic tool runs fixture tag validation
    Then the Critic uses the convention path to validate tags
    And missing tags produce MEDIUM-priority Builder action items as normal

#### Scenario: Per-Feature Test Fixtures URL Overrides Convention Path
    Given a feature has `> Test Fixtures: /tmp/custom-fixture-repo`
    And a bare git repo exists at `.purlin/runtime/fixture-repo`
    When the Critic resolves the fixture repo for the feature
    Then it uses `/tmp/custom-fixture-repo` (the per-feature override)
    And the convention path is not consulted

#### Scenario: Config fixture_repo_url Overrides Convention Path
    Given a feature spec contains a fixture tag section with declared tags
    And `.purlin/config.json` has `fixture_repo_url` set to `/tmp/alt-fixture-repo`
    And a bare git repo exists at both `/tmp/alt-fixture-repo` and `.purlin/runtime/fixture-repo`
    When the Critic resolves the fixture repo for the feature
