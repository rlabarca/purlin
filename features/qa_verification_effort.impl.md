...
*   **[CLARIFICATION]** The output schema includes both the spec-required simplified keys (`auto`, `manual`) and the pre-existing granular subcategories (`web_test`, `manual_interactive`, `manual_visual`, `manual_hardware`, `total_auto`, `total_manual`). The granular keys are kept for backward compatibility with the CDD dashboard's `_qa_badge_html()` tooltip breakdown. The `auto` key counts @auto-tagged QA scenarios only; `manual` key equals `total_manual` (sum of all manual subcategories). (Severity: INFO)
