...
- `instructions/PURLIN_BASE.md` is the SOLE instruction file. The plugin agent definition (`agents/purlin.md`) incorporates its content — no separate loading mechanism needed.
...
#### Scenario: Plugin agent definition incorporates PURLIN_BASE.md @auto

    Given agents/purlin.md exists
    When the agent definition content is examined
    Then it contains the PURLIN_BASE.md instruction content
    And it serves as the sole base instruction source for the Purlin agent

## Regression Guidance
- Verify PURLIN_BASE.md does not reference old role names (PM, Engineer) except in transition context
- Verify the instruction file loads correctly via the plugin agent definition
- Verify override file sections are not empty (template should have placeholder comments)
- Verify PURLIN_BASE.md content is incorporated in agents/purlin.md (plugin agent definition)
