...
**[DISCOVERY] [ACKNOWLEDGED]** The inline copy of agent_instructions in `features/release_checklist_core.md` Section 2.7 originally used the old "Insert" wording. That file is PM-owned. PM mode has since updated it to "Prepend" — both the authoritative source (`tools/release/global_steps.json`) and the inline spec copy are now consistent. (Severity: HIGH — resolved)
