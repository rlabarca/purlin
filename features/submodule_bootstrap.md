...
*   **Architect Launcher Logic:**
    ...
    3. Concatenate in order: `<framework>/instructions/HOW_WE_WORK_BASE.md`, `<framework>/instructions/ARCHITECT_BASE.md`, `.purlin/HOW_WE_WORK_OVERRIDES.md` (if exists), `.purlin/ARCHITECT_OVERRIDES.md` (if exists).
...
*   **Warning:** If `.purlin` appears in the project's `.gitignore`, the script MUST print a warning that `.purlin/` should be tracked (committed) and NOT gitignored.
...
*   **Current Behavior:** The script looks for config at `$DIR/../../.purlin/config.json` (the standalone path, where `$DIR` is the tool's directory).
*   **Required Behavior:** If the standalone path does not exist, the script MUST try the submodule path at `$DIR/../../../.purlin/config.json` (one level higher, for when tools are at `<project_root>/agentic-dev/tools/cdd/`).
...
*   **Launcher Export:** All generated launcher scripts (`run_architect.sh`, `run_builder.sh`, `run_qa.sh`) MUST export `PURLIN_PROJECT_ROOT` set to the absolute path of the consumer project root (i.e., `$SCRIPT_DIR`). This variable is the authoritative project root for all tools invoked during the session.
*   **Python Tool Detection:** All Python tools (`tools/critic/critic.py`, `tools/cdd/serve.py`) MUST check the `PURLIN_PROJECT_ROOT` environment variable first. If set and the path exists, use it as the project root without climbing. If not set, fall back to directory-climbing detection. Note: Graph generation is now part of the CDD tool (`serve.py`), not a separate `generate_tree.py`.
*   **Climbing Priority Reversal:** When using the directory-climbing fallback (no `PURLIN_PROJECT_ROOT`), Python tools MUST try the FURTHER path first (`../../../.purlin/config.json` -- the submodule layout where `tools/` is 3 levels deep) before the nearer path (`../../.purlin/config.json` -- standalone layout). This prevents the submodule's own `.purlin/` from shadowing the consumer project's config.
*   **Shell Tool Detection:** Shell-based tool scripts (`tools/critic/run.sh`, `tools/cdd/start.sh`) MUST check `PURLIN_PROJECT_ROOT` before falling back to relative path climbing. When set, they MUST derive config paths from `$PURLIN_PROJECT_ROOT/.purlin/config.json`.
...
*   **Runtime Directory:** Log files and PID files MUST be written to `<project_root>/.purlin/runtime/` instead of alongside the tool scripts:
    - `tools/cdd/cdd.log` -> `.purlin/runtime/cdd.log`
    - `tools/cdd/cdd.pid` -> `.purlin/runtime/cdd.pid`
*   **Cache Directory:** Generated data files MUST be written to `<project_root>/.purlin/cache/` instead of alongside the tool scripts:
    - `tools/cdd/feature_status.json` -> `.purlin/cache/feature_status.json`
    - `.purlin/cache/feature_graph.mmd` (Mermaid export, produced by CDD graph generation)
    - `.purlin/cache/dependency_graph.json` (dependency graph, produced by CDD graph generation)
    - `tools/critic/.cache/*.json` -> `.purlin/cache/critic/*.json`
*   **Directory Auto-Creation:** Tools MUST create `runtime/` and `cache/` (and any needed subdirectories) under `.purlin/` if they do not exist.
*   **Gitignore Update:** The bootstrap script's recommended gitignore entries (Section 2.7) MUST include `.purlin/runtime/` and `.purlin/cache/`.

### 2.13 Python Tool Config Resilience
*   **Error Handling:** All Python tools that read `.purlin/config.json` MUST wrap `json.load()` in a `try/except` block handling `json.JSONDecodeError`, `IOError`, and `OSError`.
...
*   **Scope:** `tools/cleanup_orphaned_features.py` and any future utility scripts MUST use the same project root detection as other Python tools (Section 2.11: `PURLIN_PROJECT_ROOT` env var first, then climbing fallback).
...
*   **Submodule-Specific Test Scenarios:** The test harness MUST exercise scenarios that are unique to the submodule environment:
    ...
    - Python tools resolve the consumer project root, not the submodule root, via both `PURLIN_PROJECT_ROOT` and climbing fallback (Section 2.11).
    - Generated artifacts are written to `.purlin/runtime/` and `.purlin/cache/`, not inside the submodule `tools/` directory (Section 2.12).
...
#### Scenario: Bootstrap a Fresh Consumer Project
    ...
    Then .purlin/ is created with ARCHITECT_OVERRIDES.md, BUILDER_OVERRIDES.md, QA_OVERRIDES.md, HOW_WE_WORK_OVERRIDES.md, and config.json
    And config.json contains "tools_root": "agentic-dev/tools"
    And .purlin/.upstream_sha contains the current submodule HEAD SHA
...
#### Scenario: Gitignore Warning
    Given .purlin is listed in the project's .gitignore
    When the user runs bootstrap.sh
    Then a warning is printed that .purlin should be tracked
...
#### Scenario: Tool Start Script Config Discovery (Submodule Layout)
    ...
    And .purlin/config.json exists at the project root with cdd_port 8086
...
#### Scenario: Tool Start Script Config Discovery (Standalone Layout)
    ...
    And .purlin/config.json exists with cdd_port 9086
    When tools/cdd/start.sh is run
...
#### Scenario: Config JSON Validity After Bootstrap
    ...
    And no .purlin/ directory exists at the project root
    When the user runs "agentic-dev/tools/bootstrap.sh"
    Then .purlin/config.json is valid JSON (parseable by python3 json.load)
...
#### Scenario: Launcher Scripts Export Project Root
    ...
    Then the script contains an export of PURLIN_PROJECT_ROOT
...
#### Scenario: Python Tool Uses PURLIN_PROJECT_ROOT
    ...
    And the project root contains .purlin/config.json
    When a Python tool (critic.py, serve.py, generate_tree.py) resolves its project root
    Then it uses $PURLIN_PROJECT_ROOT as the project root
    And it reads config from $PURLIN_PROJECT_ROOT/.purlin/config.json
...
#### Scenario: Python Tool Climbing Prefers Submodule Layout
    ...
    And both <project_root>/.purlin/config.json and <project_root>/agentic-dev/.purlin/config.json exist
    When the Python tool uses climbing fallback to find config
    Then it discovers <project_root>/.purlin/config.json (submodule/further path)
    And it does NOT use <project_root>/agentic-dev/.purlin/config.json (standalone/nearer path)
...
#### Scenario: Python Tool Survives Malformed Config
    Given .purlin/config.json contains invalid JSON (e.g., missing comma, truncated)
...
#### Scenario: Cleanup Script Uses Project Root Detection
    Given the project uses agentic-dev-core as a submodule at "agentic-dev/"
    And PURLIN_PROJECT_ROOT is set to the project root
