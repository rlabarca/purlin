...
- `pl-run.sh` (launcher script). **Generated by `tools/init.sh`.** The migration means updating the launcher template in `init.sh` so that newly generated launchers call `resolve_config.py <role>` instead of using the inline `python3 -c "import json; ..."` pattern that reads `config.json` directly. Existing consumer launcher scripts must be regenerated (via `init.sh --launcher-only` or full re-init) to pick up the change.
- `tools/cdd/scan.sh`
