...
Terminal identity is now set by the `purlin:start` and `purlin:mode` skills running inside a Claude Code session. The retired shell launchers (`pl-run.sh` and role-specific variants) have been replaced by the plugin skill model.

### Current Identity Flow

1. **`purlin:start`** -- Sets terminal identity for the initial mode (default: Engineer) during session entry.
