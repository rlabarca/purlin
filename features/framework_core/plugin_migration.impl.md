...
- [IMPL] Created `skills/resume/SKILL.md` — session entry point replacing pl-run.sh, with checkpoint recovery, mode activation, worktree entry, YOLO support, and hook integration table.
...
- [IMPL] Deleted `pl-run.sh` (replaced by purlin:resume).
...
- [IMPL] Updated CLAUDE.md for plugin model (purlin:resume, purlin:help).
...
- [IMPL] Updated `templates/CLAUDE.md` — removed legacy agent references (`Architect`, `Builder`, legacy role boundaries), replaced `/pl-resume` → `purlin:resume`, `/pl-help` → `purlin:help`, added plugin-model language and mode guard reference.
