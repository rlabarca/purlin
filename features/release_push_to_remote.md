...
5. Checks if `.purlin/runtime/active_branch` exists and is non-empty. If it does, and the current branch matches the value in that file, PM mode warns: "Current branch is a collaboration branch (`<branch>`). Pushing directly may bypass the collaboration workflow. Confirm to proceed."
...
8. Checks that the selected remote is not ahead of the local branch. If the remote has commits not present locally, PM mode reports the divergence and halts until the user resolves it.

### 2.2 Force-Push Prohibition

PM mode MUST NOT force-push without explicit user confirmation. If the push would require `--force` or `--force-with-lease`, PM mode warns the user and awaits explicit approval before proceeding.

### 2.3 Push Execution

On confirmation, PM mode executes: `git push <remote> <branch> && git push <remote> --tags`

The `<remote>` and `<branch>` are the values confirmed by the user in Section 2.1 (either the defaults or user-selected alternatives). Both commits and tags are pushed. If either command fails, PM mode reports the error and does not mark the step complete.
...
#### Scenario: Remote is ahead of local
    ...
    When PM mode executes the purlin.push_to_remote step
...
#### Scenario: Step is disabled
    ...
    When PM mode executes the release checklist
...
#### Scenario: Release push from collaboration branch shows branch confirmation
    ...
    And PM mode must confirm before proceeding
