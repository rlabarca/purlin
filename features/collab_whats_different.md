...
- **Bootstrap-installed launchers:** Root-level files matching `pl-run-architect.sh`, `pl-run-builder.sh`, `pl-run-qa.sh`.
- **Isolation launchers:** Root-level files matching `pl-run-<name>-architect.sh`, `pl-run-<name>-builder.sh`, `pl-run-<name>-qa.sh`, where `<name>` is 1-12 alphanumeric/hyphen/underscore characters (generated by `create_isolation.sh`).
