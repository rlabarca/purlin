...
- **Bootstrap-installed launchers:** Root-level files matching `run_architect.sh`, `run_builder.sh`, `run_qa.sh`.
- **Isolation launchers:** Root-level files matching `run_<name>_architect.sh`, `run_<name>_builder.sh`, `run_<name>_qa.sh`, where `<name>` is 1-12 alphanumeric/hyphen/underscore characters (generated by `create_isolation.sh`).
...
#### Scenario: Purlin Infrastructure Files Classified as purlin_config
    ...
    Given a commit range that modifies run_builder.sh, run_feat1_architect.sh, and .claude/commands/pl-status.md
    When the extraction tool categorizes the changed files
    Then run_builder.sh is categorized as purlin_config
    And run_feat1_architect.sh is categorized as purlin_config
