...
**Output:** `assets/workflow-animation.gif` (generated by `tools/release/generate_workflow_animation.py`)
...
- `tools/release/generate_workflow_animation.py` — Builder-owned.
  ...
  "steps": [
    {
      ...
      "code": "python3 tools/release/generate_workflow_animation.py",
      "agent_instructions": "Run `python3 tools/release/generate_workflow_animation.py` from the project root. Confirm exit code 0, that `assets/workflow-animation.gif` was created or updated, and that `README.md` contains exactly one reference to `assets/workflow-animation.gif` under the `## How It Works` heading."
    }
  ]
}
```
...
```
python3 tools/release/generate_workflow_animation.py
```
...
When `python3 tools/release/generate_workflow_animation.py` is executed from the project root,
...
When `python3 tools/release/generate_workflow_animation.py` is executed,
...
When `python3 tools/release/generate_workflow_animation.py` is executed on macOS,
