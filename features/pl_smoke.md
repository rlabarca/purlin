...
When a project has **5+ completed features** but **zero smoke-tier classifications** (no entries in the tier table and no `_smoke.json` files), display a one-time orientation block during `/pl-verify` or `/pl-complete`:
...
they protect your critical path.
  ...
  /pl-smoke suggest     — find candidates
  /pl-smoke <feature>   — promote a feature

Recommended: /pl-smoke suggest
━━━━━━━━━━━━━━━━━━━━━━━━
```
...
**Placement:** After Phase A summary in `/pl-verify`, or after marking `[Complete]` via `/pl-complete` when the threshold is first reached.
...
**`/pl-status` display:** When smoke candidates exist, show them in the QA section:
```
Smoke candidates (unclassified):
  agent_launchers_common — 4 dependents
```

No action is required — this is informational. QA sees it naturally and can promote via `/pl-smoke` when ready.
...
#### Scenario: First-time orientation shown when no smoke features exist
    ...
    And it suggests /pl-smoke suggest as the next step
