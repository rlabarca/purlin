...
**[DISCOVERY]** [SPEC_PROPOSAL] Instruction files `instructions/BUILDER_BASE.md` and `instructions/QA_BASE.md` still reference the retired `/pl-work-push` and `/pl-work-pull` commands (6 occurrences in BUILDER_BASE, 5 in QA_BASE). These were renamed to `/pl-local-push` and `/pl-local-pull` as part of the `agent_launchers_multiuser` tombstone retirement. The Architect should update both instruction files to use the new command names in: (1) the startup print command table, (2) the shutdown protocol section, and (3) the authorized slash commands list. (Severity: HIGH)
