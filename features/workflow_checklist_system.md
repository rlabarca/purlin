...
*   **Branch inference:** `infer_role_from_branch()` maps `spec/*` → architect, `impl/*` → builder, `qa/*` → qa. Returns None for unrecognized branches, causing the CLI to exit with an error message.
- **Branch naming:** `/pl-work-push` and `/pl-work-pull` infer role from branch using same prefix map as `serve.py` (`spec/*` → architect, `build/*` → builder, `qa/*` → qa). Update `infer_role_from_branch()` to map `build/*` → builder (was `impl/*`).
