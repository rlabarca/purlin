...
*   **[IMPL]** Added CDD launchers (`pl-cdd-start.sh`, `pl-cdd-stop.sh`) to stale artifact cleanup in: skill file Step 8, `tools/init.sh` refresh stale loop, and `tools/migration/migrate.py` `_OLD_LAUNCHERS` list. Added orphaned command file detection to skill file Step 8 — compares local `pl-*.md` files against submodule to catch commands removed in past releases.
