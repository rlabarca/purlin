...
The skill reads the role-appropriate command table from `references/{role}_commands.md` and prints the correct variant based on the current branch (main, collaboration, or isolated). This is the same logic used by the Startup Print Sequence and `/pl-resume`.
