...
Distinct from `/pl-discovery`, which records project-level bugs.
...
- **Environment**: OS, current mode (Engineer/PM/QA/none — read from session state, not legacy role markers), branch, `tools_root`
...
- **Relevant files**: if the failing command is a known skill, include the skill file path (`.claude/commands/pl-<name>.md`). If a feature was being worked on, include the feature spec path.
