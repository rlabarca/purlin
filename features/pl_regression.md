...
Bare `/pl-regression` (no subcommand) MUST auto-detect the natural next step and execute it. The detection logic scans project state and picks the first matching rule:
...
Next: /pl-regression           (auto-detects next step)
  or: /pl-regression run       (re-run suites)
```

This allows a new user to repeatedly invoke `/pl-regression` to walk through the full author → run → evaluate cycle without knowing the subcommand names.
...
#### Scenario: Bare invocation auto-detects author step
    ...
    When /pl-regression is invoked with no subcommand
