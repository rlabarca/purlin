...
*   **Spec Formatting Exemption:** Commits that only change spec formatting (indentation, Gherkin keyword casing, Critic parsing fixes, whitespace normalization) without altering behavioral content MUST NOT trigger lifecycle resets. The committing agent MUST include a `[Spec-FMT]` trailer in the commit message to signal this exemption. The CDD MUST treat `[Spec-FMT]` identically to `[QA-Tags]` when computing lifecycle reset detection — if all commits since the last status commit contain `[Spec-FMT]` (or `[QA-Tags]`, or a combination), the lifecycle is preserved. This prevents re-verification cycles caused by non-behavioral maintenance edits.
...
*   **Metadata Tag Display:** Metadata blockquotes (`> Key: Value` lines at the top of feature files -- Label, Category, Prerequisite, Web Test, Web Start, and any other `> Key: Value` patterns) MUST be extracted from the markdown and displayed in a dedicated metadata area between the header/tabs and the body content.
...
*   **Side Effect:** Regenerates `.purlin/cache/feature_status.json` (the internal lifecycle-based artifact consumed by the Critic) and `.purlin/cache/status.json` (the full API response, identical to the `/status.json` web endpoint schema). Both files are written on every status regeneration (CLI invocation without `--graph`, HTTP API request, and dashboard page load). The `--graph` mode does NOT regenerate these cache files — it only outputs the dependency graph. The `status.json` file enables agents to read current project status as a file without depending on the web server.
