# Feature: Engineer Agent Launcher

> Label: "Tool: Engineer Agent Launcher"
...
Role-specific launcher configuration for Engineer mode (`pl-run-builder.sh`). Inherits all shared launcher mechanics from `agent_launchers_common.md`.
...
*   The Engineer uses `find_work: true` and `auto_start: false` by default, consistent with other agent roles.
