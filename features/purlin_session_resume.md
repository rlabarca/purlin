# Feature: purlin:resume Session Checkpoint and Recovery Skill

> Label: "Agent Skills: Common: purlin:resume Session Resume"
...
The `purlin:resume` skill is the **entire startup flow** for the Purlin agent. The startup protocol (PURLIN_BASE.md §5) is a single line: "Run `purlin:resume`." All startup logic — merge recovery, terminal identity, command hints, scanning, work discovery, and mode activation — lives here. This prevents startup and resume flows from drifting apart.

There are three entry points that all converge on `purlin:resume`:
1. **Cold start via plugin** (`claude` launches with plugin auto-activated → `purlin:start` → `purlin:resume`): Fresh session, no checkpoint, instructions from agent definition.
2. **After `/clear` → `purlin:resume`**: Mid-session recovery, no checkpoint, instructions may be compressed out.
3. **After `purlin:resume save` → `/clear` → `purlin:resume`**: Warm resume, checkpoint exists, instructions may be compressed out.

When invoked without a checkpoint, `purlin:resume` runs a cold start: scan the project, present work via `purlin:status`, and activate the appropriate mode. When a checkpoint exists, it restores the previous session's state and resumes work. The `purlin:resume save` subcommand captures session state for recovery.
...
### 2.2 Save Mode (`purlin:resume save`)
...
### 2.3 Restore Mode (`purlin:resume`)
...
**Merge breadcrumb check:** Glob `.purlin/cache/merge_pending/*.json`. If any breadcrumbs exist, run the merge recovery protocol (Section 2.4) before proceeding. This check is essential because `purlin:resume` may be invoked standalone (after `/clear`) when no startup protocol ran.
...
This step runs ONLY when the system prompt does not contain the Purlin agent instructions (i.e., the plugin agent definition was not loaded). This is the sole instruction-loading mechanism for `purlin:resume` — no separate "startup briefing" step exists.
...
**This step is the shared path for BOTH normal startup and context recovery.** PURLIN_BASE.md §5 delegates entirely to `purlin:resume` — there is no separate implementation of work discovery in the startup protocol.
...
If `PURLIN_SESSION_ID` is not set (e.g., manual `purlin:resume` outside a launcher session), fall back to the **resolved config**: `.purlin/config.local.json` if it exists, otherwise `.purlin/config.json`. Extract `find_work` and `auto_start` from `agents.purlin`.
...
This is the path taken on fresh launch AND on `/clear` without `purlin:resume save`. The agent starts in open mode with no work plan — it discovers work fresh via scan.
...
### 2.4 Merge Recovery Mode (`purlin:resume merge-recovery`)
...
#### Scenario: Save writes PID-scoped checkpoint file @auto
    ...
    When the agent invokes purlin:resume save
...
#### Scenario: Restore with PID-scoped checkpoint re-enters mode @auto
    ...
    When the agent invokes purlin:resume
...
#### Scenario: Restore checks merge breadcrumbs in Step 0 @auto
    ...
    When the agent invokes purlin:resume (restore mode)
    Then merge recovery runs before checkpoint detection
    And the breadcrumb is processed before proceeding to Step 1

#### Scenario: Identity set once in Step 6 with branch context @auto

    Given a cold start with no checkpoint and default_mode null
    And the current branch is "main"
    When the agent completes purlin:resume
    Then the terminal identity is set to "Purlin(main) | <project>" in Step 6
    And the identity was NOT set earlier in the flow

#### Scenario: Identity reflects checkpoint mode with branch context @auto

    Given a checkpoint exists with mode "engineer"
    And the current branch is "main"
    When the agent completes purlin:resume
    Then the terminal identity is set to "Eng(main) | <project>" in Step 6
    And the identity was NOT temporarily set to "Purlin(main) | <project>" before Step 6

#### Scenario: Restore prints command hint not full table @auto

    Given any purlin:resume invocation (cold or warm)
    When the agent reaches Step 3
    Then the output contains "Use purlin:help for commands"
...
#### Scenario: Cold start runs purlin:status for work discovery @auto

    Given no checkpoint file exists
    When the agent invokes purlin:resume (cold start)
    Then scan.sh is run
    And purlin:status interprets the results
...
#### Scenario: Invalid argument prints error @auto
    ...
    When the agent invokes purlin:resume with an invalid argument
...
#### Scenario: Merge recovery resolves pending merges @auto
    ...
    When the agent invokes purlin:resume merge-recovery
...
#### Scenario: Concurrent terminals do not collide on checkpoints @auto
    ...
    When terminal A invokes purlin:resume save
    And terminal B invokes purlin:resume save
