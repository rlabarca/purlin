# Feature: purlin:whats-different Command

> Label: "Agent Skills: Common: purlin:whats-different What's Different"
...
The `purlin:whats-different` agent command generates a plain-English summary of what's different between local main and the remote collab branch. It is a standalone command usable by any role from the main checkout. When invoked with an active Purlin mode (PM, Engineer, or QA), it produces a **role-specific briefing** that answers what the reader specifically needs to care about, followed by the standard file-level digest.
...
- If the file is absent or empty, the command aborts with a message directing the user to create or join a collaboration branch via `purlin:remote branch create <name>` or `purlin:remote branch join <name>`.
