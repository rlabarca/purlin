...
> Label: "Agent Skills: Common: purlin:status Status Check"
...
When a mode is active, `purlin:status` MUST limit output to that mode's work items only. When no mode is active (open mode), show all work items across all modes (current behavior).
...
#### Scenario: All roles can invoke the command
    ...
    When the agent invokes purlin:status
...
#### Scenario: QA section shows regression failures with hint
    ...
    And the hint "Run purlin:regression to update" is shown

#### Scenario: Mode-scoped output uses focused scan flags

    Given an active PM mode session
    When purlin:status invokes scan.sh
