...
8. Update the terminal identity to reflect the current mode on the source branch (no longer in a worktree). Unified format: `<short_mode>(<branch>) | <project>` — e.g., `Eng(main) | purlin`.
