...
|---|---|---|
| *(none)* | `purlin:toolbox` | Guided interactive menu (Section 2.2) |
| `list` | `purlin:toolbox list` | Show all tools grouped by category |
| `run` | `purlin:toolbox run <tool> [tool2 ...]` | Execute one or more tools sequentially |
| `create` | `purlin:toolbox create` | Create a new project tool interactively |
| `edit` | `purlin:toolbox edit <tool>` | Edit a project or community tool |
| `copy` | `purlin:toolbox copy <purlin-tool>` | Copy a purlin tool to project for customization |
| `add` | `purlin:toolbox add <git-url>` | Download a community tool from a git repo |
| `pull` | `purlin:toolbox pull [tool]` | Update community tool(s) from source repo |
| `push` | `purlin:toolbox push <tool> [git-url]` | Push a tool to its source repo or a new repo |
| `delete` | `purlin:toolbox delete <tool>` | Delete a project or community tool |

### 2.2 Guided No-Args Menu

When `purlin:toolbox` is invoked with no subcommand, display:
...
*   **Purlin tool (consumer project):** Block with message: `"Purlin tools are read-only in consumer projects. Use 'purlin:toolbox copy <tool>' to create an editable project copy."` Purlin tools are distributed via the plugin and are read-only.
...
*   **Community tool:** Read the tool definition from `.purlin/toolbox/community/<tool_id>/tool.json`. Warn: `"Local edits will diverge from upstream. Next 'purlin:toolbox pull' will detect the conflict."` Present fields for editing. Write changes back.
...
*   Confirm: `"Copied 'purlin.<name>' to project tool '<new_id>'. You can now edit it with 'purlin:toolbox edit <new_id>'."`
...
Path resolution per `references/path_resolution.md`. Purlin tools are at `${CLAUDE_PLUGIN_ROOT}/scripts/toolbox/purlin_tools.json`. Project and community tools are at `.purlin/toolbox/` in the project root.

### 2.11 Mode and Activation

*   `purlin:toolbox` is available in all modes (shared).
...
#### Scenario: No-args shows guided menu
    ...
    Given the user invokes "purlin:toolbox" with no subcommand
...
#### Scenario: List shows all categories
    ...
    And each tool is shown in a markdown table with id, name, and full description
...
#### Scenario: Run multiple tools sequentially
    ...
    When the user runs "purlin:toolbox run verify_zero_queue doc_consistency_check"
    Then both tools execute in the order specified

#### Scenario: Create project tool

    Given no tool with id "my_audit" exists
    When the user runs "purlin:toolbox create" and provides id "my_audit", friendly_name "My Audit", description "Custom audit"
...
#### Scenario: Edit blocked for purlin tool in submodule context
    ...
    And the user runs "purlin:toolbox edit purlin.verify_zero_queue"
    When the skill resolves the tool
    Then the message "Purlin tools are read-only in consumer projects. Use 'purlin:toolbox copy ...' to create an editable project copy." is displayed
...
#### Scenario: Copy purlin tool to project
    ...
    When the user runs "purlin:toolbox copy purlin.verify_zero_queue" and accepts id "verify_zero_queue"
...
#### Scenario: Delete project tool with preview
    ...
    When the user runs "purlin:toolbox delete my_audit"
...
#### Scenario: Delete blocked for purlin tool in submodule context
    ...
    Given the project uses purlin as a git submodule
    And the user runs "purlin:toolbox delete purlin.verify_zero_queue"
    When the skill resolves the tool
...
#### Scenario: Run tool with no instructions or code
    ...
    When the user runs "purlin:toolbox run <tool>"
