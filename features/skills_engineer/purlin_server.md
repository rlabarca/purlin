...
> Label: "Agent Skills: Engineer: purlin:server Dev Server Management"
...
- Engineer mode owns the command. QA mode can invoke `purlin:server` to start/stop servers for verification (cross-mode: run-only) but cannot modify application code.
...
#### Scenario: Role gate rejects non-Engineer/QA invocation
    ...
    When the agent invokes purlin:server
    Then the command responds with a redirect message

#### Scenario: Port conflict selects alternate port

    Given port 3000 is already in use
    When purlin:server starts a dev server with default port 3000
...
#### Scenario: Cleanup stops server and removes state
    ...
    When purlin:server stop is invoked
