...
- If any merge produces a conflict, the launcher MUST stop immediately, report the conflicting files, preserve the unmerged branches (do NOT delete them), write a `merge_failed` flag to `.purlin/runtime/`, and exit with a message listing the preserved branches and recovery instructions. The EXIT trap MUST check for this flag and skip branch deletion when set. This ensures the work is recoverable via `git cherry-pick` or `git merge` after manual conflict resolution.
- After successful merges, clean up worktree branches.
- **Deferred status tracking:** The orchestrator monitors each parallel Builder process. As soon as an individual Builder exits successfully (exit code 0), the orchestrator records the completion to `.purlin/runtime/phase_status.json` (a gitignored runtime file). The orchestrator MUST NOT update `delivery_plan.md` or make git commits while worktree branches are live — doing so causes guaranteed merge conflicts because worktree branches forked from main before those commits. After all worktree branches are merged back to main, the orchestrator applies all deferred status updates from `phase_status.json` to `delivery_plan.md` in a single atomic commit. The canvas engine reads phase status from process liveness checks and `phase_*_meta` files (not from the delivery plan), so real-time progress display is unaffected. For Builders that exit non-zero, the phase remains `[IN_PROGRESS]` until the evaluator decides to retry or stop. All git commands in this path MUST suppress output per Section 2.17 (Orchestrator command suppression).
