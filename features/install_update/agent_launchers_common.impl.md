...
Submodule equivalents are produced by `scripts/mcp/bootstrap.py` (which superseded the retired `bootstrap.sh`) and follow the same structure with a plugin-specific `CORE_DIR` path.
