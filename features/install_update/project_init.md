...
> Prerequisite: instructions/PURLIN_BASE.md (Section 6: Layered Instruction Architecture)
...
Users start sessions with `claude` (the plugin auto-activates via MCP). Session entry is handled by the `purlin:resume` skill, and mode switching by `purlin:mode`.
...
5.  **[Retired]** Launcher script generation is no longer performed. Sessions are started via `claude` with the plugin auto-activating. The `purlin:resume` skill handles session entry; `purlin:mode` handles mode switching.
...
13. **CLAUDE.md Installation:** Install or update `CLAUDE.md` at the project root using marker-based block insertion (`<!-- purlin:resume -->` / `<!-- purlin:end -->`). Source template is `purlin-config-sample/CLAUDE.md.purlin`. Preserves user content outside the markers. See `context_recovery_hook.md` Section 2.3 for the full protocol.
...
#### Scenario: Full Init Installs Session Recovery Hook
    ...
    And the hook command echoes the purlin:resume recovery instruction
...
#### Scenario: Refresh Mode Never Touches Config or Overrides @auto
    ...
    And .purlin/PURLIN_OVERRIDES.md has custom content
    When the user runs "purlin/tools/init.sh"
