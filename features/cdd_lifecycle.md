...
- Create `cdd_start.sh` as a symlink to `tools/cdd/start.sh` at the repository root.
- Create `cdd_stop.sh` as a symlink to `tools/cdd/stop.sh` at the repository root.
- Both symlinks are committed to git.
- Consumer projects continue using their own `purlin_cdd_start.sh` / `purlin_cdd_stop.sh` (generated by bootstrap, unchanged by this feature).

### 2.9 Duplicate Prevention

- `start.sh` refuses to start a second instance for the same project root. If a CDD process for this project is already running (detected via `ps`), the script prints the existing URL and exits 0.
- This is idempotent: running `start.sh` multiple times always results in exactly one server and always prints a usable URL.
