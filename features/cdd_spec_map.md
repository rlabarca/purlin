...
*   **Large Graph Performance:** For projects with 100+ features, the graph MUST remain interactive (pan/zoom responsive within 100ms). For projects exceeding 500 features, the layout engine MAY switch to a simplified rendering mode (e.g., collapsed category groups with expand-on-click) to maintain viewport usability. The graph MUST NOT cause browser tab crashes or memory exhaustion regardless of feature count.
