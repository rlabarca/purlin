...
*   **Repo URL Resolution:** The Critic resolves the fixture repo URL using a two-tier lookup: (1) per-feature `> Test Fixtures:` metadata, then (2) project-level `fixture_repo_url` in `.purlin/config.json`. Per-feature metadata takes precedence. Relative paths in either source are resolved against `PURLIN_PROJECT_ROOT`.
*   **Tags declared without repo URL:** When a feature declares fixture tags but neither per-feature `> Test Fixtures:` metadata nor project-level `fixture_repo_url` config exists, the Critic MUST generate a MEDIUM-priority Architect action item with category `fixture_no_repo_url`: `"Feature <name> declares fixture tags but no fixture repo URL is configured (add > Test Fixtures: metadata or set fixture_repo_url in config.json)"`. This is an Architect item because the URL configuration is a specification/infrastructure decision.
*   **Tags declared but repo not found:** When a feature declares fixture tags and a repo URL resolves (per-feature or project-level), but the `fixture list` command fails (repo not found, unreachable, or invalid path), the Critic MUST generate a MEDIUM-priority Builder action item with category `fixture_repo_unavailable`: `"Fixture repo for <name> is unreachable at <url> — run the setup script to create it"`. This is a Builder item because creating/initializing fixture repos is an implementation task.
