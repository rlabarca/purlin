...
- **Status:** RESOLVED
- **Resolution:** Added a "Path Resolution" section to `skills/update/SKILL.md` that instructs the agent to use `PURLIN_PROJECT_ROOT` env var as the primary project root detection mechanism, with directory-climbing as fallback. Added 4 tests to `tests/pl_update_purlin/test_command.py` verifying presence and placement.
