...
Branch collaboration extends Purlin's local isolation model to multi-machine workflows via named branches managed through `purlin:remote`. Any valid git branch name can be used as a collaboration branch. One branch is active at a time per machine. Remote `main` is untouched during active collaboration -- all sync flows through the active branch.
...
- Branches are created and managed via `purlin:remote branch` (create, join, leave, list).
...
- `purlin:remote push` and `purlin:remote pull` read this file to determine the target branch.
...
All collaboration commands (`purlin:remote push`, `purlin:remote pull`) use the collaboration branch as their integration target. During an active branch, the local machine checks out that branch, making push and pull symmetric same-branch operations.
...
- `purlin:remote push` and `purlin:remote pull` resolve the collaboration branch per Section 2.4: if `.purlin/runtime/active_branch` is present and non-empty, the collaboration branch is that value; otherwise it is `main`.
...
Local collaboration branch -> remote (via `purlin:remote push`). During an active branch, the collaboration branch is the value from `.purlin/runtime/active_branch`.

### 2.7 Fetch-Before-Push

Always `git fetch` before pushing. If the local collaboration branch is BEHIND or DIVERGED relative to the remote, block and require `purlin:remote pull` first.

### 2.8 Merge-Not-Rebase for Collaboration Branch

`purlin:remote pull` uses `git merge` (not rebase) on the local collaboration branch. The collaboration branch is a shared integration branch -- rebasing rewrites commits other contributors depend on.

If `git merge` encounters conflicts during `purlin:remote pull`, the command halts with file-level conflict context (which files conflict, the merge state). Conflict resolution is a human responsibility -- the agent does not auto-resolve. After manual resolution, the user runs `purlin:remote pull` again to verify clean state.
...
- **Create** (`purlin:remote branch create <name>`): Creates the branch from HEAD and pushes to remote. The local machine checks out the new branch. The branch becomes immediately discoverable by other collaborators via `git fetch`.
- **Join** (`purlin:remote branch join <name>`): Fetches the remote branch, checks out the branch locally, and sets it as active.
- **Leave** (`purlin:remote branch leave`): Checks out the stored base branch (or `main` if absent) and clears the active branch. Does NOT delete any branches. The branch remains joinable.
...
- **Merge to main:** When the release is ready, the collaboration branch is merged to `main` via a standard `git merge` (not rebase, consistent with Section 2.8). This merge is performed by the human or PM as part of the release checklist, not by `purlin:remote push`.
- **Post-merge push:** After merging to `main`, the `purlin.push_to_remote` release step pushes `main` to the remote. The active branch file should be cleared (`purlin:remote branch leave`) before this step.
...
- `purlin:remote add` is the standalone subcommand for configuring a remote: scans for hosting hints (SSH keys, credential helpers, hosting CLIs), prompts for a git remote URL, executes `git remote add` or `git remote set-url`, and verifies connectivity via `git ls-remote`. It does not push or pull.
- `purlin:remote push` prints: "No git remote configured. Run `purlin:remote add` to set up a remote first." and exits with code 1.
- `purlin:remote pull` prints: "No git remote configured. Run `purlin:remote add` to set up a remote first." and exits with code 1.
...
The same confirmation pattern applies to `purlin:remote pull` on first pull from a remote+branch pair. First pull is detected by checking whether a merge-base exists between the local branch and the remote tracking branch. The confirmation displays remote details and incoming commit count.
...
- Running `purlin:remote push` or `purlin:remote pull` from a branch that does not match the resolved collaboration branch
- `git checkout main` while a branch is active -- use `purlin:remote branch leave` to return to main
- Manual editing of `.purlin/config.json` to set up branch collaboration (use `purlin:remote add` for remote config)

## Scenarios

No automated or manual scenarios. This is a policy anchor node -- its "scenarios" are
process invariants enforced via `purlin:remote` subcommand guards and runtime state files.
