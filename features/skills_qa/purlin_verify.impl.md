# Implementation Notes: purlin_verify
...
The `purlin:verify` skill file (`skills/verify/SKILL.md`) is a QA agent instruction file implementing the interactive feature verification workflow described in `features/purlin_verify.md`.
...
Scoped mode (`purlin:verify <feature>`) previously did not run `scan.sh`, causing the agent to read inline file tags for lifecycle determination. Since lifecycle is git-commit-based (status commits), this defaulted to TODO for features that had no inline tag — even when a `[Ready for Verification]` status commit existed on the current branch. This was reported by a user running Engineer and QA in separate terminal sessions.
