...
This is an alternative *execution method* for Manual Scenarios and Visual Specs, not a new classification. The Automated/Manual/Visual taxonomy in feature specs stays unchanged. Features opt in via a `> Web Test:` metadata annotation. Non-web features (CLI tools, file-based systems) remain manual via `purlin:verify`.
...
- Features without this annotation are not eligible for `purlin:web-test` and continue using `purlin:verify` (manual).
- Feature files MAY include a `> Web Start: <command>` blockquote metadata line (e.g., `> Web Start: purlin:server`). When present and the server is not reachable during pre-flight, the skill invokes this command to start the server before proceeding.
...
- If no arguments, auto-discover web-testable features: run `tools/cdd/scan.sh`, read `purlin:status`, identify TESTING features, read each spec for `> Web Test:` metadata.
- Only features with `> Web Test:` are eligible. Skip all others silently.
- If no web-testable features are found, inform the user and suggest `purlin:verify` instead.
...
- For inconclusive items: list them with recommendation for manual verification via `purlin:verify`.
- Commit discovery entries with message format: `qa(<scope>): [BUG] - web-verify findings`.

### 2.11 Completion Gate

- **QA Agent invocation:** If all scenarios and visual items pass (zero failures, zero inconclusive), prompt: "All web verification passed. Run `purlin:complete <name>` to mark done?" If confirmed, run `purlin:complete`.
- **Engineer invocation:** If all pass, print summary only. Do not mark complete. Suggest the QA agent run `purlin:complete`.

### 2.12 Fixture-Backed Testing

When a feature has `> Web Test:` metadata and the project has a fixture repo (resolved via the three-tier lookup: per-feature `> Test Fixtures:` metadata → config `fixture_repo_url` → convention path `.purlin/runtime/fixture-repo`), `purlin:web-test` can execute scenarios against fixture-provided project states rather than the live project. This solves the "complex setup" problem -- scenarios requiring specific branch states, collaboration setups, or config values get their preconditions from immutable fixture tags.
...
1. **Rename** skill file `skills/aft-web.md` to `skills/web-test/SKILL.md`. Update all internal references: `> AFT Web:` to `> Web Test:`, `> AFT Start:` to `> Web Start:`, `purlin:aft-web` to `purlin:web-test`.
...
6. **Delete** old skill file `skills/web-verify.md` if it still exists.
...
- `references/visual_spec_convention.md` -- Document that `> Web Test:` enables automated visual verification via Playwright MCP. Update on-demand loader notice to include `purlin:web-test`.
- `references/visual_verification_protocol.md` -- Add Section 5.4.7: Playwright MCP automated alternative referencing `purlin:web-test`. Update on-demand loader notice to include `purlin:web-test`.
- `agents/purlin.md` -- Add `purlin:web-test` to the authorized commands list (Section 3.0). Add brief reference in Section 5.4 noting the automated alternative for web-testable features.
- `agents/purlin.md` -- Add `purlin:web-test` to the authorized commands list (Section 2.0). Add brief reference in Section 5.3 (Verify Locally) noting web verification as a pre-TESTING validation option.
- `references/qa_commands.md` -- Add `purlin:web-test [name]` entry to both command table variants (Main, Collab), placed after `purlin:verify`.
- `references/builder_commands.md` -- Add `purlin:web-test [name]` entry to both command table variants (Main, Collab), placed after `purlin:propose`.
...
#### Scenario: Auto-discover web-testable features
    ...
    Given purlin:status shows features in TESTING state
    And some features have `> Web Test:` metadata and others do not
    When `purlin:web-test` is invoked without arguments
...
#### Scenario: URL override from argument
    ...
    When `purlin:web-test feature_name http://localhost:3000` is invoked
...
#### Scenario: Playwright MCP not available triggers headless auto-setup
    ...
    When `purlin:web-test` is invoked
...
#### Scenario: Dynamic port resolution from runtime port file
    ...
    When `purlin:web-test` resolves the URL for that feature
...
#### Scenario: Server auto-start when not reachable
    ...
    Given a feature has `> Web Test: http://localhost:9086`
    And the feature has `> Web Start: purlin:server`
    And no server is reachable at the resolved URL
    When `purlin:web-test` performs the liveness check
    Then the skill invokes `purlin:server` to start the server
...
#### Scenario: Manual scenario PASS recorded correctly
    ...
    When `purlin:web-test` executes the scenario
...
#### Scenario: Inconclusive step handled gracefully
    ...
    When `purlin:web-test` cannot automate that step
    Then the step is marked INCONCLUSIVE
    And the summary recommends manual verification via `purlin:verify`
...
#### Scenario: Visual spec items verified via screenshot analysis (no Figma MCP)
    ...
    When `purlin:web-test` navigates to the screen and takes a screenshot
...
#### Scenario: Figma-triangulated verification with all sources agreeing
    ...
    When `purlin:web-test` performs triangulated verification
...
#### Scenario: Figma-triangulated verification detects token drift
    ...
    When `purlin:web-test` performs token verification
    Then the token entry is recorded as DRIFT
    And the output shows Figma=20px App=16px

#### Scenario: Three-source report format

    Given `purlin:web-test` has completed triangulated verification for a feature
...
#### Scenario: Regression scope respected
    ...
    When `purlin:web-test` is invoked for that feature
...
#### Scenario: QA completion gate prompts for completion
    ...
    Then the skill prompts to run `purlin:complete <name>`
...
#### Scenario: Engineer completion gate is summary only
    ...
    And the skill suggests QA run `purlin:complete`
...
#### Scenario: Instruction files updated with web-verify references
    ...
    Then `purlin:web-test` appears in both QA and Engineer authorized command lists
    And `purlin:web-test [name]` appears in all variants of both command tables
...
#### Scenario: Fixture-backed server started for scenario with fixture tag
    ...
    When `purlin:web-test` processes that scenario
...
#### Scenario: Fixture checkout failure marks scenario inconclusive
    ...
    When `purlin:web-test` attempts to check out the fixture
...
#### Scenario: Fixture cleanup after scenario completion
    ...
    When `purlin:web-test` moves to the next scenario
...
#### Scenario: Legacy pl-web-verify references fully removed
    ...
    Given the pl-web-test skill file exists at `skills/web-test/SKILL.md`
    When a search is performed for "pl-web-verify" or "Web Testable" across all non-release-note files
    Then zero matches are found
    And the old skill file `skills/web-verify.md` does not exist
