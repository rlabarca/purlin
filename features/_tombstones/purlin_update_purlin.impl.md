...
*   **[IMPL]** Updated skill file to use tag-based version resolution (`git describe --tags --abbrev=0 origin/main`) instead of `origin/main` HEAD. Added optional `<version>` argument for explicit tag/branch targeting. Updated step 1 (Fetch and Version Check), step 3 (Advance Submodule), Options section, and Example usage per spec §2.1/§2.3/§2.5 changes.
*   **[IMPL]** Updated docs (index.md, common-commands-guide.md, installation-guide.md) to reflect tag-based targeting and the new `[version]` argument in skill listings.
*   **[CLARIFICATION]** Auto-fix iteration 1: restored `--preflight-only` flag to `scripts/init.sh`. The flag was removed per a `project_init` discovery but `pl_update_purlin` spec §2.6 requires it for prerequisite checking after submodule advance. Added `PREFLIGHT_ONLY` variable, CLI case branch, and early exit after preflight checks.
*   **[IMPL]** Added CDD launchers (`pl-cdd-start.sh`, `pl-cdd-stop.sh`) to stale artifact cleanup in: skill file Step 8, `scripts/init.sh` refresh stale loop, and `scripts/migration/migrate.py` `_OLD_LAUNCHERS` list. Added orphaned command file detection to skill file Step 8 — compares local `pl-*.md` files against submodule to catch commands removed in past releases.
*   **Agent Skill Architecture:** This feature is implemented as a Claude Code slash command (`skills/update-purlin/SKILL.md`), not as a shell script or Python tool. The command file is a prompt that instructs the agent to perform the update workflow interactively. Tests verify command file structure, referenced infrastructure, and behavioral invariants.
*   **Test Approach:** `tests/pl_update_purlin/test_command.py` validates the command file contains required references for each automated scenario (structural change detection keywords, merge strategy labels, stale artifact names, infrastructure file existence, etc.). This follows the same pattern as other agent skill tests (e.g., `tests/pl_start/test_command.py`).
...
**Source:** purlin:spec-code-audit --deep (item #22)
**Severity:** MEDIUM
...
**Source:** purlin:spec-code-audit --deep (M16)
**Severity:** MEDIUM
**Details:** Spec §2.13 requires the skill to use `PURLIN_PROJECT_ROOT` (env var) for project root detection with directory-climbing as fallback. The skill file (`skills/update-purlin/SKILL.md`) contains no mention of either mechanism.
**Suggested fix:** Add a Path Resolution section to the skill file referencing `PURLIN_PROJECT_ROOT` and climbing fallback, matching the pattern in `skills/resume/SKILL.md`.
