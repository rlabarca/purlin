...
### Orphan Detection
Orphan detection is handled by the scan engine (`tools/cdd/graph.py`), which reports orphaned features in `.purlin/cache/dependency_graph.json` under the `orphans` key. The standalone `cleanup_orphaned_features.py` was removed (2026-03-26) as redundant.
...
- **2.7 Orphan Detection**: Handled by scan engine (`graph.py`). Standalone script removed.
