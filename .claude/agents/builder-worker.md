...
- **Steps 0-2 only** from `/pl-build`. Do NOT run Step 3 (verification) or Step 4 (status tags). The main Builder session handles verification after merging all parallel branches.
...
3. Execute `/pl-build` Steps 0-2 (Pre-Flight, Plan, Implement).
4. Commit your implementation.
5. Return your results to the main Builder session.
