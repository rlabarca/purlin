...
3. Check the dependency graph for cycles via `.agentic_devops/cache/dependency_graph.json`.
4. Verify the active release spec (`features/release_*.md`) is marked `[Complete]`.
5. Check `PROCESS_HISTORY.md` is up to date.
6. Consult `.agentic_devops/release/config.json` for the project-specific release step sequence and work through each enabled step in order.
