...
**Invariants are externally-sourced and locally immutable.** No mode (Engineer, PM, QA) can write to `features/i_*.md` files. Changes come ONLY via `/pl-invariant add`, `/pl-invariant add-figma`, or `/pl-invariant sync`.
...
Use /pl-invariant add <repo-url> to import, or create a local anchor without the i_ prefix.
```
...
1. **Determine the correct prefix.** If the topic argument starts with `i_`, redirect to `/pl-invariant` (see Invariant Anchors above). If the topic already starts with `arch_`, `design_`, `policy_`, `ops_`, or `prodbrief_`, use that prefix. If it does NOT match any recognized prefix, prompt the user:
   ```
   What type of anchor?
