...
class TestTestQualityAudit(unittest.TestCase):
    """Tests for Critic Section 2.17: Test Quality Audit Trail.

    Covers all 6 scenarios from features/critic_test_quality_audit.md.
    """
    ...
    def setUp(self):
        ...
        self.features_dir = os.path.join(self.root, 'features')
        os.makedirs(self.features_dir)
        # Feature with 3 automated scenarios
...
"""
        self.feature_path = os.path.join(
            self.features_dir, 'sample_feature.md')
        with open(self.feature_path, 'w') as f:
            f.write(self.feature_content)
        self._orig_features_dir = critic.FEATURES_DIR
        critic.FEATURES_DIR = self.features_dir

    def tearDown(self):
        import critic
        critic.FEATURES_DIR = self._orig_features_dir
        shutil.rmtree(self.root)

    def _make_result(self, lifecycle_state='testing'):
        """Build a minimal feature_result dict."""
        cdd_status = None
        if lifecycle_state == 'testing':
            cdd_status = {
                'features': {
                    'testing': [{'file': 'features/sample_feature.md'}],
                    'complete': [],
                    'todo': [],
                },
            }
            ...
            cdd_status = {
                'features': {
                    'testing': [],
                    'complete': [{'file': 'features/sample_feature.md'}],
                    'todo': [],
                },
            }
            ...
            cdd_status = {
                'features': {
                    'testing': [],
                    'complete': [],
                    'todo': [{'file': 'features/sample_feature.md'}],
                },
            }
        result = {
            'feature_file': 'features/sample_feature.md',
            'spec_gate': {'status': 'PASS', 'checks': {}},
            'implementation_gate': {
                'status': 'PASS',
                'checks': {
    ...
    def test_missing_audit_generates_low_action_item(self):
        ...
        companion = os.path.join(
            self.features_dir, 'sample_feature.impl.md')
        with open(companion, 'w') as f:
            f.write('## Implementation Notes\nSome notes.\n')
        result, cdd = self._make_result('testing')
        items = generate_action_items(result, cdd)
        audit_items = [
            i for i in items['builder']
            if i['category'] == 'missing_test_quality_audit']
        self.assertEqual(len(audit_items), 1)
        self.assertEqual(audit_items[0]['priority'], 'LOW')
        self.assertIn('sample_feature', audit_items[0]['description'])
        self.assertIn('policy_test_quality.md', audit_items[0]['description'])

    def test_present_audit_suppresses_action_item(self):
        ...
        with open(companion, 'w') as f:
            f.write(
                ...
                '- Scenario: First -> ALIGNED\n')
        result, cdd = self._make_result('testing')
        items = generate_action_items(result, cdd)
        audit_items = [
            i for i in items['builder']
            if i['category'] == 'missing_test_quality_audit']
        self.assertEqual(len(audit_items), 0)

    def test_missing_companion_triggers_action_item(self):
        """Scenario: Missing companion file triggers action item.

        Given a feature with automated scenarios
        And builder status is DONE
        And no companion file exists
        When the Critic runs
        Then a LOW-priority Builder action item is generated.
        """
        # No companion file created — default state
        result, cdd = self._make_result('testing')
        items = generate_action_items(result, cdd)
        audit_items = [
            i for i in items['builder']
    ...
    def test_zero_automated_scenarios_is_exempt(self):
...
#### Scenario: Manual check
    Given a manual setup
    When user inspects
    Then behavior is correct
"""
        manual_path = os.path.join(
            self.features_dir, 'manual_only.md')
        with open(manual_path, 'w') as f:
            f.write(manual_only)
        result, cdd = self._make_result('testing')
        result['feature_file'] = 'features/manual_only.md'
        items = generate_action_items(result, cdd)
        audit_items = [
            i for i in items['builder']
            if i['category'] == 'missing_test_quality_audit']
    ...
    def test_builder_todo_is_exempt(self):
        """Scenario: Feature with builder TODO is exempt.

        Given a feature with 3 automated scenarios
        And builder status is TODO (lifecycle=todo)
        When the Critic runs
        Then no missing_test_quality_audit action item is generated.
        """
        result, cdd = self._make_result('todo')
        items = generate_action_items(result, cdd)
        audit_items = [
            i for i in items['builder']
            if i['category'] == 'missing_test_quality_audit']
    ...
    def test_audit_does_not_affect_implementation_gate(self):
        ...
        And the missing_test_quality_audit item appears only in action items.
        """
        result, cdd = self._make_result('testing')
        # Verify impl gate is still PASS
        self.assertEqual(
            result['implementation_gate']['status'], 'PASS')
        items = generate_action_items(result, cdd)
        # The action item exists
        audit_items = [
            i for i in items['builder']
            if i['category'] == 'missing_test_quality_audit']
        self.assertEqual(len(audit_items), 1)
        # But impl gate was not modified
        self.assertEqual(
            result['implementation_gate']['status'], 'PASS')
