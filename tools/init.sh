...
# Copies the canonical pl-run.sh from the framework and substitutes the
# CORE_DIR path for the consumer project's submodule location.
# This ensures one source of truth — the framework's pl-run.sh.
# Usage: generate_purlin_launcher <output_file>
generate_purlin_launcher() {
    local OUTPUT_FILE="$1"
    local SOURCE_LAUNCHER="$SUBMODULE_DIR/pl-run.sh"

    if [ ! -f "$SOURCE_LAUNCHER" ]; then
        echo "ERROR: Canonical launcher not found at $SOURCE_LAUNCHER" >&2
        return 1
    fi

    # Copy canonical launcher, substituting the CORE_DIR path for the submodule.
    # The canonical launcher has: CORE_DIR="$SCRIPT_DIR/purlin"
    # Consumer projects need:     CORE_DIR="$SCRIPT_DIR/<submodule_name>"
    # When the submodule is named "purlin" (the default), this is a no-op.
    sed "s|^CORE_DIR=\"\$SCRIPT_DIR/purlin\"|CORE_DIR=\"\$SCRIPT_DIR/$SUBMODULE_NAME\"|" \
        "$SOURCE_LAUNCHER" > "$OUTPUT_FILE"
