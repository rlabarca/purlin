...
#
# Flags:
#   --regenerate-launchers  Also regenerate launcher scripts in refresh mode
...
###############################################################################
QUIET=false
REGEN_LAUNCHERS=false

for arg in "$@"; do
    case "$arg" in
        --quiet) QUIET=true ;;
        --regenerate-launchers) REGEN_LAUNCHERS=true ;;
...
# Generate the project-root shim (purlin_init.sh).
generate_shim() {
    local REMOTE_URL
    ...
    cat > "$PROJECT_ROOT/purlin_init.sh" << SHIM_EOF
#!/bin/bash
# purlin_init.sh — Purlin project initialization shim.
...
SHIM_EOF
    chmod +x "$PROJECT_ROOT/purlin_init.sh"
}
...
create_cdd_symlinks() {
    ...
    # purlin_cdd_start.sh
    if [ -L "$PROJECT_ROOT/purlin_cdd_start.sh" ]; then
        local current_target
        current_target="$(readlink "$PROJECT_ROOT/purlin_cdd_start.sh")"
        if [ "$current_target" != "$start_target" ]; then
            ln -sf "$start_target" "$PROJECT_ROOT/purlin_cdd_start.sh"
        fi
    elif [ ! -e "$PROJECT_ROOT/purlin_cdd_start.sh" ]; then
        ln -s "$start_target" "$PROJECT_ROOT/purlin_cdd_start.sh"
    fi

    # purlin_cdd_stop.sh
    if [ -L "$PROJECT_ROOT/purlin_cdd_stop.sh" ]; then
        local current_target
        current_target="$(readlink "$PROJECT_ROOT/purlin_cdd_stop.sh")"
        if [ "$current_target" != "$stop_target" ]; then
            ln -sf "$stop_target" "$PROJECT_ROOT/purlin_cdd_stop.sh"
        fi
    elif [ ! -e "$PROJECT_ROOT/purlin_cdd_stop.sh" ]; then
        ln -s "$stop_target" "$PROJECT_ROOT/purlin_cdd_stop.sh"
    fi
}
...
" 2>/dev/null)" || true
    ...
    say ""
    say "  purlin_init.sh          Run anytime to refresh"
    say "  ./run_architect.sh      Start Architect session"
    say "  ./run_builder.sh        Start Builder session"
    say "  ./run_qa.sh             Start QA session"
    say "  ./purlin_cdd_start.sh   Start CDD dashboard"
    say "  ./purlin_cdd_stop.sh    Stop CDD dashboard"
...
###############################################################################
    ...
    SHIM_FILE="$PROJECT_ROOT/purlin_init.sh"
    ...
    SYMLINK_NOTE=""
    REPAIRED=0
    if [ ! -e "$PROJECT_ROOT/purlin_cdd_start.sh" ]; then
        REPAIRED=$((REPAIRED + 1))
    fi
    if [ ! -e "$PROJECT_ROOT/purlin_cdd_stop.sh" ]; then
        REPAIRED=$((REPAIRED + 1))
    fi
    create_cdd_symlinks
