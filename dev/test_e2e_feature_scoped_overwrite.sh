...
# ==========================================================================
# Phase A — Write login proofs, then signup proofs → both READY
# ==========================================================================
...
login_ready=false
signup_ready=false
echo "$STATUS_A" | grep -q "login: READY" && login_ready=true
...
# ==========================================================================
# Phase B — Overwrite login proofs (feature-scoped) → both still READY
# ==========================================================================
...
echo "$STATUS_B" | grep -q "login: READY" && login_ready_b=true
echo "$STATUS_B" | grep -q "signup: READY" && signup_ready_b=true

phase_b_ok=false
if $login_ready_b && $signup_ready_b; then
  echo "    Phase B PASS: login READY (new entries), signup READY (untouched)"
  phase_b_ok=true
else
...
phase_c_ok=false
# login should show 1/2 rules proved (not READY, not carrying over old PROOF-2)
if echo "$STATUS_C" | grep -q "login: 1/2 rules proved"; then
  # Also verify signup is still READY
  if echo "$STATUS_C" | grep -q "signup: READY"; then
    echo "    Phase C PASS: login shows 1/2 (purged old proof), signup still READY"
    phase_c_ok=true
  else
    echo "    Phase C FAIL: signup not READY"
  fi
else
