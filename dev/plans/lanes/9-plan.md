...
3. Orchestrator, in the main tree: `python3 scripts/init/scaffold.py --update --yes
   --project-root .` (the first real run of the upgrade path on this repository: migrates
   `.purlin/config.json` to the B2 shape at `gate: recorded`, deletes the pre-commit shim
...
6. The user pushes `evidence-workflow`; `purlin.yml` runs the Linux and Windows jobs; both
   ...
   item 4).
7. Approve in the agent what the review list shows; `bash dev/bump_version.sh 0.10.0`;
