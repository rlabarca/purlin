...
4. **Configures the MCP server** — writes `.mcp.json` so Claude Code starts Purlin's MCP server (the `sync_status`, `purlin_config`, and `drift` tools).
...
your-project/
  ...
  .mcp.json                # Purlin MCP server config
