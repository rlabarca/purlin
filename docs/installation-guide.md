...
The remaining fields are optional and a full `purlin:init` never writes them: `audit_llm`, `audit_llm_name`, `audit_criteria`, `audit_criteria_pinned`, `platforms` and `quality_gate`. Where each one's owner and default live is [Drift Criteria](../references/drift_criteria.md), section "Config Field Ownership", which is the table this list is read against.

`platforms` is the registry the `@on(<platform-id>)` proof tags resolve against; the family ids `windows`, `macos` and `linux` work with no config at all, and an entry is what adds a version, an architecture or a runner:
