...
- **`.purlin/report-data.js` is committed.** It is the project digest: coverage and drift data that travels with the repo, so QA, PM and compliance readers can open the dashboard without running Purlin tools. The pre-commit hook `purlin:init` installs regenerates it on every commit. Set the mode with `purlin:init --digest`: `auto`, `warn` or `off`. Only the hooks compute the drift block; a `purlin:status` call carries forward the block already in the file rather than blanking it, so a status call on an unchanged tree leaves the committed digest alone.
