...
git checkout --theirs specs/auth/login.proofs-default.json
git add specs/auth/login.proofs-default.json
...
Choose "external" and enter your LLM CLI command:

```
gemini -m pro -p "{prompt}"
```

The audit criteria are sent verbatim to the external LLM. Different model weights, different training data, different biases. The report shows which LLM performed the audit.

When agent teams are active, the lead relays external audit findings to the builder teammate. The builder never talks to the external LLM directly — the lead handles the translation.
