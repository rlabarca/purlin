...
- Figma URL pasted (figma.com/design/...) → use the Figma MCP to read the design. If the user asks to build from it, build it. Suggest creating a design invariant (`purlin:invariant sync figma <url>`) and specs afterward — but don't block the work.
