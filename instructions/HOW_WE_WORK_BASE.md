...
*   **QA-owned testing:** QA Scenarios (`### QA Scenarios` section). Scenarios are classified as `auto` (`@auto` tag -- QA authors regression JSON and runs via harness runner), `manual` (default -- requires human interaction), or `force-manual` (`@manual` tag -- always human, QA never proposes automation). QA follows the auto-first protocol (QA_BASE Section 3.3): automate `@auto` scenarios first, then verify manual scenarios. Features with BOTH Unit Tests and QA Scenarios: QA auto-credits the Builder's Unit Test results and presents only the QA items.
