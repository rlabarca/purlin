...
> **Regression result validity:** A `regression.json` with `status: "PASS"` where source files have NOT been modified since the result was written is a VALID pass. Do NOT re-run it, do NOT flag it as "prior run; re-run needed," and do NOT require fresh execution. Only STALE (source modified since last run), FAIL, and NOT_RUN results require action. "It passed in a prior session" is not a reason to re-run — the staleness check (source mtime vs result mtime) is the sole arbiter of whether a re-run is needed.
...
*   **Step 4 — Classify untagged scenarios:** For each QA scenario with NO tag (neither `@auto` nor `@manual`), QA attempts to automate it BEFORE running manually:
    ...
    *   **Commit format:** When committing tag classification changes, the commit message MUST include `[QA-Tags]` as a trailer (e.g., `qa(<feature>): classify N QA scenarios [QA-Tags]`). This signals CDD to skip lifecycle reset for this commit. Similarly, `[Spec-FMT]` exempts formatting-only spec edits from resets (see HOW_WE_WORK_BASE Section 4.4). When evaluating lifecycle state, treat both tags as equivalent reset exemptions.
...
*   **Step 5a — Phase A Checkpoint (MANDATORY HARD GATE):** Finalize every feature that passed automated verification during Phase A. This step runs at TWO points:
    *   **Checkpoint A:** After Steps 1–5 complete — finalize AUTO features verified by web tests, @auto scenarios, and visual smoke.
    *   **Checkpoint B:** After in-session regression suites pass — finalize features (both AUTO and TODO) whose regression suites ran in-session and passed. This checkpoint fires BEFORE the external agent_behavior test gate. Do NOT wait for external tests to finalize features that are already clean.

    **CRITICAL: Committing regression artifacts is NOT finalization.** The Critic tracks lifecycle via status commit messages (`[Complete features/<name>.md] [Verified]`), NOT via file changes. Without the status tag commits, features stay AUTO/TODO forever regardless of how many test results are committed. You MUST execute ALL substeps IN ORDER:

    1.  **Commit regression artifacts FIRST:** Commit any new or modified `tests/<feature>/regression.json`, `tests/qa/scenarios/*.json`, and `features/*.discoveries.md` files. Commit message: `qa: commit Phase A regression results and scenario updates`.
    2.  **Commit status tags (ONE PER FEATURE):** For EACH feature where all automated tests passed and zero discoveries exist, commit: `git commit --allow-empty -m "status(<scope>): [Complete features/<name>.md] [Verified]"`. This is the commit that changes lifecycle state. Without it, the feature remains in its current QA status. This applies to both `qa_status: AUTO` features and `qa_status: TODO` features whose @auto/regression work is fully satisfied. Do NOT batch multiple features into one status commit — one commit per feature.
    3.  **Update CDD status (HARD GATE):** Run `{tools_root}/cdd/status.sh`. Do NOT proceed to Phase B or present the manual checklist until this completes. The dashboard must reflect completions NOW.
    4.  **Verify completions:** After the Critic runs, confirm the finalized features no longer show as AUTO or TODO in the QA column. If any still show AUTO, the status tag commit was missed — go back to substep 2.
    5.  **Clean workspace check:** Run `git status`. All Phase A artifacts must be committed. No uncommitted files should carry forward.
    6.  If zero manual items remain and no pending external tests, skip directly to Session Conclusion (Section 6).
    **Hard rule:** The external agent_behavior test gate (which blocks Phase B) does NOT block Step 5a. Features that passed in-session automated verification are finalized immediately regardless of whether external tests are still pending for other features.
