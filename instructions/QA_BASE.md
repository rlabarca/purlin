...
*   **Step 5a — Phase A Checkpoint (MANDATORY):** Finalize every feature that passed automated verification during Phase A. This step runs at TWO points:
    ...
    *   **Checkpoint B:** After in-session regression suites pass — finalize features (both AUTO and TODO) whose regression suites ran in-session and passed. This checkpoint fires BEFORE the external agent_behavior test gate. Do NOT wait for external tests to finalize features that are already clean.
    At each checkpoint, execute all substeps:
    1.  **Mark completions:** For each feature where all automated tests passed and zero discoveries exist, commit: `git commit --allow-empty -m "status(<scope>): [Complete features/<name>.md] [Verified]"`. This applies to both `qa_status: AUTO` features and `qa_status: TODO` features whose @auto/regression work is fully satisfied.
    2.  **Commit regression artifacts:** Commit any new or modified `tests/<feature>/regression.json`, `tests/qa/scenarios/*.json`, and `features/*.discoveries.md` files. Commit message: `qa: commit Phase A regression results and scenario updates`.
    3.  **Update CDD status:** Run `{tools_root}/cdd/status.sh` to regenerate the Critic report. The dashboard must reflect completions immediately — not after the entire session ends.
    4.  **Clean workspace check:** Run `git status`. All Phase A artifacts must be committed. No uncommitted files should carry forward.
    5.  If zero manual items remain and no pending external tests, skip directly to Session Conclusion (Section 6).
