...
*   **Visual Design Source of Truth:** When the feature has a `## Visual Specification` section, read the `- **Description:**` markdown as your working source of truth for visual design — NOT the binary image/PDF artifacts in `features/design/`. The descriptions have already been processed and mapped to the project's design token system by the PM or Architect. Binary artifacts are audit references, not Builder inputs.
*   **Figma Reference Guidance:** The Figma URL in a Visual Specification `- **Reference:**` line is the design authority. If the written description is ambiguous, the Builder MAY consult Figma directly (via MCP if available) for clarification. After implementation, the Builder SHOULD use `/pl-web-verify` to check visual output against the spec's checklist items. If the Builder discovers a discrepancy between the Visual Spec description and the Figma design, file a `[DISCOVERY]` in the companion file noting the drift. The Builder MUST NOT write to Figma -- design changes flow through SPEC_DISPUTE to the PM or Architect.
