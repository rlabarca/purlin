...
*   **Scope clarification:** This priority order governs how the Builder chooses implementation values (CSS properties, layout decisions, spacing). It does NOT restrict AFT verification -- when `/pl-aft-web` runs, it reads Figma MCP directly for three-source comparison regardless of implementation phase.
...
*   **Unit/Integration Testing (MANDATORY):**
    ...
    *   **Behavior Over Artifacts Mandate:** Tests MUST execute the code path under test and verify the outcome. A test that passes by inspecting source code, grepping function bodies, or checking that an artifact merely exists (non-empty file, non-zero length) without validating its content is not a behavioral test -- it is a checkbox and is treated as missing coverage. **The test must break when the behavior breaks.** If you can delete the implementation and the test still passes, the test is worthless.
*   **Automated Feedback Testing (MANDATORY for AFT-eligible features):** For features with AFT metadata (`> AFT Web:`, `> AFT API:`, etc.), the Builder MUST run the corresponding AFT tool before the status tag commit. `/pl-aft-web` is the current implementation for `> AFT Web:` features; other AFT tools will be added as they are built. If the AFT reports failures, fix them before proceeding to Step 4. Features without any AFT metadata skip this step. The Builder MUST iterate `/pl-aft-web` until all visual spec items and manual scenarios PASS (zero BUG verdicts). If Figma MCP is available, the three-source triangulated comparison (Figma + Spec + App) is the acceptance bar. STALE and SPEC_DRIFT verdicts are PM action items, not Builder blockers -- log them as `[DISCOVERY]` and continue. BUG verdicts (code doesn't match spec/Figma) MUST be fixed before the status tag commit.
