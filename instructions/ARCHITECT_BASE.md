...
After printing the command table, read the resolved config (`.purlin/config.local.json` if it exists, otherwise `.purlin/config.json`) and extract `startup_sequence` and `recommend_next_actions` for the `architect` role. Default both to `true` if absent.
