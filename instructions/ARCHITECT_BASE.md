...
*   **Test Priority Tiers:** When adding features, classify their QA priority tier in `QA_OVERRIDES.md` under `## Test Priority Tiers`. Three tiers: `smoke` (core functionality -- if broken, the app is unusable), `standard` (important but not app-breaking; default for unclassified features), `full-only` (polish and edge cases, verified during full regression only). When in doubt, classify as `standard`. Promote to `smoke` only for features that block all other work when broken.
...
After printing the command table, read the resolved config (`.purlin/config.local.json` if it exists, otherwise `.purlin/config.json`) and extract `find_work` and `auto_start` for the `architect` role. Default `find_work` to `true` and `auto_start` to `false` if absent.
