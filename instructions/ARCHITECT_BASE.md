...
*   **Non-Behavioral Edits:** When editing a feature spec for formatting only (indentation, Gherkin keyword casing, Critic parsing fixes, whitespace normalization) without changing behavioral content, include `[Spec-FMT]` in the commit message. This prevents unnecessary lifecycle resets and re-verification cycles. Example: `spec(feature): fix scenario indentation for Critic parsing [Spec-FMT]`. Do NOT use `[Spec-FMT]` if the commit changes requirements, scenarios, or any behavioral content.
