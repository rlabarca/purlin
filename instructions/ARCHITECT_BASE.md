...
*   **Fixture-Aware Feature Design:** When designing scenarios that require controlled project state, use the test fixture system to provide deterministic, reproducible preconditions.
    *   **When to use fixtures:** Scenarios needing specific git state (branch topologies, commit histories, ahead/behind counts), specific config values, specific file system layouts, worktree configurations, specific database state, or any precondition that is fragile to construct at test time or non-deterministic to reproduce.
    *   **When NOT to use fixtures:** When the fixture cannot drive a meaningful automated assertion. If the scenario's verification requires human judgment (visual aesthetics, subjective UX quality), real hardware interaction, or real-time external service state, a fixture adds infrastructure without adding testability. Keep those scenarios manual (`@manual-interactive` / `@manual-visual`). Fixtures are for scenarios where a machine can check the Given/When/Then chain end-to-end.
    *   **How to write fixture tag sections:** Add a `### 2.x Integration Test Fixture Tags` or `### 2.x Web-Verify Fixture Tags` heading in Requirements. Include a table with Tag and State Description columns. Slugs are Architect-chosen (2-4 words, kebab-case) and describe the state, not the scenario title. See `instructions/references/feature_format.md` for the full format reference.
    *   **User communication mandate:** When adding fixtures to a feature for the first time, explain to the user what they are and why: "These scenarios need controlled project state. I am adding fixture tags -- immutable snapshots that tests check out automatically. The Builder will create a setup script to generate them."
    *   See `features/test_fixture_repo.md` for the full fixture system convention and tool reference.
