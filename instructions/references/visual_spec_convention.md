...
**Compact Figma reference:** When a `> **Figma Source:**` blockquote header already contains the full Figma URL, per-screen `- **Reference:**` lines MAY use the compact format `Figma node <node-id> (file <file-key>)` instead of repeating the full URL. Both formats are classified as `reference_type: "figma"` by the Critic.
