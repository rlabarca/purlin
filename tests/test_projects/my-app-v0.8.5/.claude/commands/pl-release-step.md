...
Manage local release steps interactively. Operates on `.purlin/release/local_steps.json` and `.purlin/release/config.json` via `${TOOLS_ROOT}/release/manage_step.py`.
...
4. Run `${TOOLS_ROOT}/release/manage_step.py create --dry-run --id "<id>" --name "<name>" --desc "<desc>" [--code "<cmd>"] [--agent-instructions "<text>"]`. Display the dry-run output.
...
5. Run `${TOOLS_ROOT}/release/manage_step.py modify <id> --dry-run [changed fields only]`. Display the dry-run output.
...
5. On confirmation: run `${TOOLS_ROOT}/release/manage_step.py delete <id>`. Commit: `git commit -m "release-step(delete): <step-id>"`.
