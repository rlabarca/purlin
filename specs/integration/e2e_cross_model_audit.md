...
- RULE-5: Custom audit LLM command in config responds to ping, config stores audit_llm and audit_llm_name, and the two-pass audit completes through the configured command
...
- PROOF-5 (RULE-5): Write config with fake LLM command; verify ping returns PURLIN_AUDIT_OK; verify config has audit_llm and audit_llm_name; run static_checks (Pass 1) then configured command (Pass 2); verify structured response fields parsed @e2e
