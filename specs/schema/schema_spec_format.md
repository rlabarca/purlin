...
- RULE-7: Feature specs use heading `# Feature: <name>`, anchor specs use `# Anchor: <name>`, invariant specs use `# Invariant: <name>`
...
- PROOF-5 (RULE-5): Create a spec with `> Requires: i_base`; run sync_status; verify the report includes `i_base/RULE-1` in the coverage output @slow
- PROOF-6 (RULE-6): Create a spec with `> Scope: scripts/gate.sh`; verify sync_status parses and displays the scope files @slow
- PROOF-7 (RULE-7): Grep `specs/**/*.md` for first-level headings; verify each starts with `# Feature:`, `# Anchor:`, or `# Invariant:`
