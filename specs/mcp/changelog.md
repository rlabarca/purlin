...
- RULE-4: proof_status entries include a `structural_only` boolean, true when the feature is READY and all proofs are structural
...
- RULE-8: File entries include `structural_drift: true` when the file's spec has `structural_only: true` and category is CHANGED_BEHAVIOR
- RULE-9: Returns a `drift_flags` array summarizing features with structural-only coverage that have changed files
...
- PROOF-3 (RULE-3): Call changelog; verify all 5 top-level JSON keys present @integration
- PROOF-4 (RULE-4): Create structural-only spec with all proofs passing; verify structural_only: true in proof_status @integration
...
- PROOF-8 (RULE-8): Create structural-only spec with changed scope file; verify structural_drift: true @integration
- PROOF-9 (RULE-9): Same setup; verify drift_flags array entry @integration
- PROOF-10 (RULE-10): Create spec with non-existent scope path; verify broken_scopes entry @integration
- PROOF-11 (RULE-11): Create a temp repo with 50+ commits, no verify or tags; call changelog; verify spec-from-code recommendation. Create repo with <30 commits; verify normal changelog @integration
