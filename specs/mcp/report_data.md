...
- RULE-29: Each feature carries an `awaiting_runner` list of `{id, tier}` for every proof it declares at a runner-gated tier with no result there. The feature's `proved`/`total` already exclude the rules those proofs are the only cover for (`sync_status` RULE-47), so without this list the payload reports a clean N/N with nothing to explain the exclusion. The list is empty, not absent, when nothing is awaiting
...
- PROOF-30 (RULE-29): Build report data for a feature declaring PROOF-1 @unit and PROOF-2 @windows with only the unit proof executed; verify its `awaiting_runner` holds exactly `{id: PROOF-2, tier: windows}`, that `total` counts 1 rather than 2, and that a feature with no runner-gated proofs carries `awaiting_runner: []` rather than omitting the key @integration
