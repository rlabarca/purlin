...
- PROOF-10 (RULE-10): Wrap the atomic move in a recording spy and set the key `key` to `val`; verify the spy saw at least 1 call, that the last call's source ends in `.tmp` and its destination is `config.local.json`, that the file on disk parses to `{"key": "val"}` and that no temporary file remains. Then, with the local file already holding `{"key": "val"}`, patch the move to raise `OSError` and set the same key to `other`; verify the file still parses to exactly `{"key": "val"}` and no temporary file is left
