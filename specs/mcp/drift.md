...
- RULE-20: `drift` takes no role argument and declares none. Its tool schema declares exactly one property, `since`, and its Python signature takes the project root and `since`. The retired parameter reached neither `_compute_drift` nor the payload: the schema advertised the enum `pm, eng, qa, all` while `sync_status`'s advertised `pm, dev, qa`, two contradictory enums for a filter nothing applied. The role belongs to the `purlin:drift` skill, which filters the report it formats from this payload, and that is where it stays
...
- PROOF-23 (RULE-20): Send `tools/list`; verify the `drift` tool's `inputSchema.properties` names exactly `['since']`, that no one of the three tools declares a `role` property, and that `inspect.signature(purlin_server.drift).parameters` names exactly `project_root` and `since`. Then send `tools/call` for `drift` with `arguments` `{"role": "eng"}` and verify the returned text parses as a JSON object rather than opening `Error running drift`, so a stale caller is ignored and not broken. A schema that still declares the enum fails the first assertion and a signature that still accepts the parameter fails the third @integration
