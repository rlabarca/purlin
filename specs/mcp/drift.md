...
>   adds what the payload already knows about states and approvals. Four role
>   views come out of the same data, because a PM, a designer, QA and an
...
- RULE-7: The report carries `since`, `commits`, `files`, `spec_changes`, `broken_scopes`, `pins`, `rule_details`, `states`, `review_list` and `roles` [risk: high] [origin: eng]
...
- RULE-14: The four role views `pm`, `design`, `qa` and `eng` are present on every report, each carrying its own keys: criteria without rules, pm-owned rules changed, engineer-added rules and pins behind for the PM; designs changed and design rules stale for the designer; approvals stale, the length of the review list and rules with no negative case for QA; files touched, rules affected, tests missing, tags missing, pins behind and re-verify pending for the engineer [risk: high] [origin: eng]
- RULE-15: A role argument narrows the answer to `since`, `role`, `view` and `commits`, and nothing else [risk: medium] [origin: eng]
- RULE-16: `rule_details` carries, per spec with a changed scope file, the spec path, the changed files, the total and unproved rule ids and the rules in rule-number order, each description cut to 200 characters on a word boundary and suffixed with an ellipsis when cut [risk: medium] [origin: eng]
...
- PROOF-1 (RULE-1): Call drift with `since` set to `--output=/tmp/x` while `subprocess.run` is wrapped by a recording spy; verify the returned JSON carries `error` `rejected since`, that the reason names both a commit count and a `YYYY-MM-DD` date, and that the spy recorded zero calls. As a control, call it with `since` `2` and verify a `commits` list comes back and the spy did record calls @integration
...
- PROOF-7 (RULE-6): Commit 12 files of differing length in one commit, then run drift with `since` `1` while `subprocess.run` is wrapped by a recording spy; verify exactly 1 recorded call carries the numbered-stat flag, that all 12 file entries carry a line count matching `^\+\d+ -\d+$`, and that each value equals the numbered stat taken for that path on its own @integration
...
- PROOF-14 (RULE-12): Check the same pinned source 3 times through one cache while `subprocess.run` is wrapped by a recording spy; verify exactly 1 recorded call lists the remote @integration
...
- PROOF-18 (RULE-16): Give the feature `ledger` 4 own rules, one of them 600 characters long and unproved, and a required anchor holding 3 rules, with passing proofs for the other 3 own rules and all 3 anchor rules; commit a change to every scope file and run drift. Verify `rule_details` for `ledger` reads 7 total rules with `RULE-4` the only unproved one and the spec path `specs/ledger/ledger.md`, that its rules list holds exactly 4 entries in rule-number order, that the 600-character description comes back as 204 characters ending in an ellipsis with its opening clause intact, and that two further runs in fresh processes return byte-identical details @integration
