...
- PROOF-17 (RULE-15): Run drift with the role `qa`; verify the report's role reads `qa` and that its view carries exactly the keys `audit`, `manual`, `review_list_size`, `rules_without_a_negative_case` and `signatures_stale` @integration
