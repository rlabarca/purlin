...
- RULE-8: File entries include `behavioral_gap: true` when the file's spec has no behavioral proofs (only structural checks) and category is CHANGED_BEHAVIOR
- RULE-9: Returns a `drift_flags` array summarizing features with no behavioral proofs (behavioral gap) that have changed files
...
- PROOF-4 (RULE-4): Create structural-only spec with all proofs passing; verify structural_checks count in proof_status and proved=0 @integration
...
- PROOF-8 (RULE-8): Create structural-only spec with changed scope file; verify behavioral_gap: true @integration
