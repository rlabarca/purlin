...
- RULE-39: sync_status reports both quality gauges. When a design cache exists it prints `Proof Design: N%` computed as PROVABLE / (PROVABLE + LOOSE + UNPROVABLE), excluding STRUCTURAL from both numerator and denominator. When no audit cache exists it distinguishes two states: a project where no proof has executed reads `Proof Integrity: no tests yet`, while one with executed proofs reads `No audit data` — reporting them identically made a deliberate spec-first project look neglected
