...
- RULE-42: The next-step directive is chosen from observable state: a rule with no proof whose feature has no existing `> Scope:` files routes to `purlin:build <name>`, while one whose scope files exist routes to `purlin:unit-test`. A feature emits exactly one directive per uncovered rule, never a duplicate
...
- PROOF-72 (RULE-42): Create a spec whose `> Scope:` names a file that does not exist and which has no proofs; call sync_status and verify the directive is `purlin:build <name>` and appears exactly once. Create the scope file and verify the directive becomes `purlin:unit-test`
