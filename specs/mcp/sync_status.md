...
- RULE-39: sync_status reports both quality gauges. When a design cache exists it prints `Proof Design: N%` computed as PROVABLE / (PROVABLE + LOOSE + UNPROVABLE), excluding STRUCTURAL from both numerator and denominator; when none exists it prints `Proof Design: not measured` rather than omitting the gauge, so a project that has never graded its descriptions is distinguishable from one where the reporter forgot. When no audit cache exists it distinguishes two states: a project where no proof has executed reads `Proof Integrity: no tests yet`, while one with executed proofs reads `No audit data` — reporting them identically made a deliberate spec-first project look neglected. Each printed percentage is followed by a parenthetical carrying its measurement coverage as `N of M measured` and that gauge's own relative age, because a score over a thin slice of the project is not a project-wide score and the two caches age independently
