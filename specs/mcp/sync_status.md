...
- RULE-7: Classifies proofs as structural (grep/existence checks) or behavioral (observable outcomes). Structural proofs are reported as "checks" and excluded from the proved/total count and READY determination. Only behavioral proofs count toward coverage
...
- PROOF-7 (RULE-7): Create a spec with all grep-based proofs; verify structural checks are reported separately and feature is not READY @integration
...
- PROOF-18 (RULE-18): Create 3 features (one READY, one partial, one with no proofs); run sync_status; verify output starts with summary table, correct ready count, and detail follows @integration
