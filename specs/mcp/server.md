...
- PROOF-9 (RULE-9): Send a read of the key `gate` and verify the answer is exactly `{"gate": "tested"}`; send a write setting it to `recorded` and read it again, verifying `{"gate": "recorded"}`; then read `.purlin/config.json` from disk and verify its gate still reads `tested` @integration
