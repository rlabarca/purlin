...
- RULE-4: A rule that no proof line names is Drafted and carries an empty proof list, so a spec cannot reach a state its evidence does not support [risk: high] [origin: eng]
...
- PROOF-4 (RULE-4): Write a spec holding `RULE-1` and an empty `## Proof` section, build the payload, and verify `RULE-1` reads an empty proof list and the state `Drafted`
