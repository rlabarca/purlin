...
- RULE-3: `test_file` is recorded relative to the project root with `/` separators on every operating system, so a backslash never reaches a proof file and the same test recorded from a subdirectory and from the root reads the same value [risk: medium] [origin: eng]
