...
>   observed, the seven fields an entry carries, the two statuses, how a rule's passed
...
- PROOF-1 (RULE-1): Write a spec holding `RULE-1` and `PROOF-1`, build the payload with an empty proof directory and verify the rule's passed cell reads `no test`; write `.purlin/runtime/proofs/foo.unit.json` holding one passing entry for `PROOF-1`, build the payload again and verify the cell reads `passed`
