...
- RULE-15: The test-writing step branches on `mutation_checks` in `.purlin/config.json`. When it is `true` the skill requires the mutation check before the commit that carries the proof, by the procedure `references/spec_quality_guide.md#mutation-check` states rather than a second copy of it, and records each mutation in the commit body, so the check is re-runnable from git history. When it is `false` the skill prints one visible line saying the check is off and how to turn it on, because an omitted check that is also silent reads as a check that passed
- RULE-16: The feature-scoped auditor is spawned only after `purlin:test` has emitted proof files: the spawn instruction sits after the `purlin:test` delegation and inside Step 4 or a later step. Audit Pass 0.5 and Pass 1 exit 2 when the feature has no proof file, so an auditor spawned in the test-writing step crashes on every fresh feature
...
- PROOF-8 (RULE-8): Extract the `## When Running as Proof Fixer` section of `skills/build/SKILL.md`; verify it reads audit findings keyed by `PROOF-ID`, instructs `Fix the test to address the specific issue`, and reports back with the literal `Fixed PROOF-N` and `Re-audit please`
...
- PROOF-15 (RULE-11): Build a proof-fixer-style changeset fixture carrying two `PROOF-N → tests/test_login.py:<line>` mappings; verify the `PROOF-[0-9]+ → <file>:<line>` mapping pattern matches, that the `── Changeset ` and `── Review ` headers are both present, and that no `── Decisions ` header appears. The same four checks run against two changesets that each break RULE-11 in exactly one way, one mapping `RULE-1 → src/login.py:12` instead of `PROOF-N` and one carrying a `── Decisions ` header alongside its `PROOF-1 →` mappings, and must reject both @e2e
- PROOF-16 (RULE-12): In the temp project after the build session, verify all four exit criteria hold: `specs/auth/login.proofs-unit.json` exists with every proof `status` equal to `pass`, the PROOF-13 changeset summary validated, `git status --porcelain` lists no `.proofs-` path and no uncommitted `.py`/`.js`/`.ts` file, and `git ls-files specs/auth/login.proofs-unit.json` returns that path @e2e
- PROOF-17 (RULE-9): Run purlin:build via claude -p with a spec containing ambiguous rules; verify agent output contains Changeset with RULE mappings, non-empty Decisions with real judgment calls, and non-empty Review with risk areas @e2e @on(claude-cli)
- PROOF-18 (RULE-10): After the agent build session, `git log -5` in the generated project contains a `feat(login):` commit subject and at least one `RULE-[1-5]` reference carried in the commit body @e2e @on(claude-cli)
- PROOF-19 (RULE-12): After the agent build session, `git status --porcelain` in the generated project lists no path containing `.proofs-` and no uncommitted `.py`, `.js` or `.ts` file other than `conftest`, and `git ls-files -- specs/` returns at least one `.proofs-` path @e2e @on(claude-cli)
...
- PROOF-22 (RULE-15): Grep `skills/build/SKILL.md` and verify both branches are present and distinguishable: the `true` branch names `references/spec_quality_guide.md#mutation-check`, requires the check before the commit, says the mutation is recorded in the commit body, and does not restate the procedure (no `watch it fail` and no `restore, re-run` in the branch); the `false` branch carries the literal line the skill prints, naming `purlin:init --mutation-checks on`. Verify the commit step also names the mutation lines, so the two halves cannot drift apart. Deleting either branch fails the proof
- PROOF-23 (RULE-16): Read `skills/build/SKILL.md`; locate the character offset of the auditor-spawn line (the line naming `purlin:purlin-auditor`) and the offset of the `purlin:test <name>` delegation line. Verify the spawn offset is the greater of the two, that the spawn instruction states it runs after `purlin:test` has emitted proof files, and that the nearest preceding `## ` heading is Step 4 or a later step. Moving the paragraph back above the delegation line fails the proof
