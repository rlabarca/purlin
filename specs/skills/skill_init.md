...
- RULE-5: The skill asks one question, what must be true before CI lets a change merge, and names exactly three answers, `passed`, `strong` and `signed`, with what CI requires under each; the only two further questions are the language of an empty repository and the signer emails under `signed` [risk: high] [origin: eng]
