...
- RULE-17: `hard_gates.md` states that receipts are issued for features reported PASSING and explains that VERIFIED is the post-receipt state, so requiring it would deadlock the first receipt. Its "What Is NOT a Gate" list records that a low Proof Design or Proof Integrity score is advisory by default and never blocks unless the project declares `quality_gate`, and names that field as the opt-in that makes the deterministic half of the two gauges a CI failure while the LLM half stays advisory. A reader counting gates has to find the opt-in in the same list that tells them the gauges do not block, or the first project to turn it on reads as a second framework gate
