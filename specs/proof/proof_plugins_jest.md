...
- RULE-3: `test_file` is recorded as the path relative to the project root (proof_common RULE-23), not to Jest's own `rootDir`, which is the directory jest was started in
...
- PROOF-3 (RULE-3): Run Jest; verify `test_file` is relative to the project root @e2e
