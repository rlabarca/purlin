...
>   `Not passed`, `Weak`, `Not signed`. The project's gate decides how far the
>   chain is read: `passed`, `strong` or `signed`. It writes nothing, prints
...
- RULE-4: A rule blocked at the strong cell is named under `Weak` with the cell's word and its reasons [risk: high] [origin: eng]
...
- RULE-11: `--json` prints the gate, the minimum, the commit, the rule counts, the three sections, the result and the exit code [risk: medium] [origin: eng]
...
- PROOF-9 (RULE-4): Run the gate at gate `strong` with no brief written for the high-risk rule; verify it exits 1, opens `Weak (1):`, reads `login RULE-2: needs a person` and names `no brief for the current hashes` @integration
- PROOF-10 (RULE-4): Write a brief whose observation reads `PROOF-2 asserts the status but never the body the rule names.` and run the gate at gate `strong`; verify it exits 1, reads `login RULE-2: needs a person` and prints that sentence @integration
...
- PROOF-25 (RULE-11): Run the gate with `--json` over a project at gate `passed` with no record; verify the JSON reads gate `passed`, result `fail`, exit 1, 2 rules, 0 met, 2 entries under `not_passed`, empty `weak` and `not_signed`, and the commit the payload named @integration
