...
1.  **No hardcoded project-root assumptions.** Python tools MUST use `PURLIN_PROJECT_ROOT` (env var) first, then climbing fallback with submodule-priority ordering (further path before nearer path). See `submodule_safety_audit.md` Section 2.2, Category 1.
...
If ANY item fails, fix it before committing. Reference `features/submodule_safety_audit.md` Section 2.2 for the full check categories.
...
*   Reference `features/submodule_safety_audit.md` Section 2.2 as the canonical submodule safety check categories.
