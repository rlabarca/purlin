...
- Run `purlin_scan` (with `only: "features,deps"`) to get current feature status.
...
*   See `${CLAUDE_PLUGIN_ROOT}/references/phased_delivery.md` Section 10.8 for the normative rules.
...
*   **B1 (Build):** Existing per-feature loop (Steps 0-3 from `purlin:build`). Each feature implemented and locally tested including web tests.
...
*   When a phase completes with deferred features, record them in the `**Deferred:**` field (e.g., `feature_c.md (architect TODO) -> Phase M`). The deferred features must also be added to the target phase's `**Features:**` line. See `${CLAUDE_PLUGIN_ROOT}/references/phased_delivery.md` Section 10.14.
