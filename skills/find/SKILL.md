---
name: find
description: Find a spec by name and show its rules' states
---
...
8 rules: 6 Approved, 1 Recorded, 1 Proof ready   strength 81%
  RULE-1  high    Approved     PROOF-1  tests/test_login.py::test_rejects_bad_password
  RULE-2  medium  Recorded     PROOF-2  tests/test_login.py::test_locks_after_five
  RULE-3  low     Proof ready  PROOF-3  no test carries this marker
```

Show the risk and the origin only when the spec carries them; under the `tested` gate both are
...
Specs (12):
  auth/ (3)
    login              8 rules, 6 Approved, strength 81%
    permissions        4 rules, Drafted
  _anchors/ (2)
...
| A rule is Tested with no record | `→ Run: purlin:verify <feature>` |
