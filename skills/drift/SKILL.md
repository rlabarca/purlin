...
Report what changed in the tree that the specs, the tests and the approvals have not caught up
with. Four views, one per role: each shows the same underlying change from the angle of the
...
purlin:drift eng                Files touched, tests missing, tags missing, pins, re-verify
...
purlin:drift qa                 Approvals stale, how long the review list is, missing cases
...
A feature showing every rule Recorded after behavioural code changed still needs a look: those
records were taken against the old behaviour.
...
drift design: 2 things to look at
  ...
  checkout RULE-4 (origin: design)  Stale: the mock it names was re-exported
```
...
drift qa: review list is 12 rules (3 high, 6 medium, 3 low)
  ...
  login RULE-3               Stale: rule text changed after approval
  billing RULE-2             Recorded, never reviewed
  export RULE-1              every proof asserts a success path; no negative case
```

## Step 4: what drift never does

It never edits a spec, a test or an approval, and it never counts `re-verify pending` as work
for a person: the code changed, the approval stands, and CI clears it on the next run. Say so
in one line rather than listing those rules as findings.
...
| Only `re-verify pending` | `→ Push; CI clears it.` |
