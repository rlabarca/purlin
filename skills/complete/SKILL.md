...
1.  **TESTING state:** Confirm the feature is in TESTING state (run `${CLAUDE_PLUGIN_ROOT}/scripts/cdd/scan.sh --only features` if needed).
...
Run `${CLAUDE_PLUGIN_ROOT}/scripts/cdd/scan.sh --only features` to confirm the feature transitions to COMPLETE.
