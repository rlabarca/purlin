...
1. Run `${CLAUDE_PLUGIN_ROOT}/scripts/cdd/scan.sh --only features` and read the JSON output.
...
5. If no web-testable features are found, inform the user: "No web-testable features found in TESTING state. Use `purlin:verify` for manual verification." and stop.
...
2. **If available:**
   ...
   b. **If the args array does NOT contain `--headless`:** Instruct the user to reconfigure:
      ...
      Then restart the Claude Code session and re-run purlin:web-test.
      ```
      Stop execution.
...
3. **If NOT available:**
   ...
   c. Inform the user: "Playwright MCP server has been configured in headless mode. Please restart the Claude Code session to load the new MCP server, then re-run `purlin:web-test`."
...
1. **Checkout:** Run `${CLAUDE_PLUGIN_ROOT}/scripts/test_support/fixture.sh checkout <repo-url> <tag>` via Bash to obtain the fixture state in a temp directory. The command prints the checkout path to stdout.
2. **Start server:** Run the feature's `> Web Start:` command with `--project-root <fixture-dir> --port 0` to serve from the fixture state and bind an ephemeral port. Read the actual port from the server's stdout.
...
5. **Cleanup:** After the scenario completes (pass or fail), stop the fixture-backed server and run `${CLAUDE_PLUGIN_ROOT}/scripts/test_support/fixture.sh cleanup <fixture-dir>` via Bash.
...
6. For steps requiring something outside the browser (file system, environment, email):
   ...
   - Otherwise mark as INCONCLUSIVE with a reason note recommending manual verification via `purlin:verify`.
...
6. **For inconclusive items:** List them with recommendation: "The following items could not be automated. Use `purlin:verify` for manual verification."
