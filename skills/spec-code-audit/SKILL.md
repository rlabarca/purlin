...
- If auto-compaction occurs, save state and instruct user to resume with `purlin:spec-code-audit --deep`.
...
- **PM FIX:** Add missing `> Prerequisite:` links to applicable scoped invariants. For stale invariants, run `purlin:invariant sync <file>` to pull the latest version. For design Token Map mismatches, update the Token Map or run `purlin:invariant sync` for the design invariant.
