...
> This file is loaded on-demand by `purlin:verify` and `purlin:web-test` when a feature has a `## Visual Specification` section.
...
If the user declines screenshots: present the full checklist and ask for a single PASS/FAIL for all visual items (current behavior, preserved as-is). In batched mode, this is the default -- visual items are numbered `[V]` entries in the main checklist and the user judges them by number alongside functional scenarios.
...
In the batched verification workflow, visual items are already integrated into the flat-numbered checklist alongside functional scenarios. Cross-feature batching is the default behavior -- all visual items across all TESTING features share a single numbering sequence.
...
For features with `> Web Test: <url>` metadata, `purlin:web-test` provides fully automated visual verification using Playwright MCP browser control tools. The agent navigates to each screen, takes screenshots, executes interactions (hover, click, theme switch), and judges each checklist item via vision analysis -- no manual screenshot provision or human confirmation required. Results are recorded as PASS/FAIL per checklist item with observation notes. Failures are recorded as `[BUG]` discoveries in the standard format.

When Figma MCP is also available and a visual spec screen has a Figma reference, `purlin:web-test` performs **Figma-triangulated verification**: comparing three independent sources (Figma design via MCP, spec Token Map + checklists, running app via Playwright) to detect discrepancies with attribution. The Figma source is read via `get_design_context` (fileKey + nodeId — returns design tokens as CSS variables, component data, and layout hints) and `get_variable_defs` (fileKey — returns resolved variable values for token comparison). For visual judgment items, `get_screenshot` (fileKey + nodeId) provides a Figma frame screenshot for vision comparison against the browser screenshot. Verdicts include PASS, BUG (app wrong -> Engineer), STALE (Figma updated -> PM re-ingest), and SPEC_DRIFT (app matches Figma but not spec -> PM sync). Token Map entries are also verified by comparing Figma design variable values (from `get_variable_defs`) against the app's computed CSS property values.

## 5.4.8 Figma-Assisted Manual Verification
For non-web-testable features, QA can still use Figma MCP during manual `purlin:verify` sessions. When a visual spec screen has a Figma reference and Figma MCP is available:
1. QA reads Figma via `get_design_context` (fileKey + nodeId) to extract expected layout, dimensions, colors, and design token CSS variables from the referenced frame. Optionally calls `get_screenshot` (fileKey + nodeId) for visual comparison.
2. The user provides a screenshot (photo, simulator capture, etc.).
3. QA vision-compares the user screenshot against the Figma screenshot (from `get_screenshot`) + spec checklists.
