...
When Figma MCP is also available and a visual spec screen has a Figma reference, `purlin:web-test` performs **Figma-triangulated verification**: comparing three independent sources (Figma design via MCP, spec Token Map + checklists, running app via Playwright) to detect discrepancies with attribution. Verdicts include PASS, BUG (app wrong -> Engineer), STALE (Figma updated -> PM re-ingest), and SPEC_DRIFT (app matches Figma but not spec -> PM sync). Token Map entries are also verified by comparing Figma design variable values against the app's computed CSS property values.
