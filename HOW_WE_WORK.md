...
The single source of truth for any project using this framework is not the code, but the **Specifications** and **Architectural Policies**. 
*   **Application Domain:** Defines the target product (stored in the host project's `features/` directory).
*   **Agentic Domain:** Defines the workflow and tools (stored in this framework's `features/` directory).
...
*   **Ownership:** `ARCHITECT_INSTRUCTIONS.md`, Architectural Policies, Feature Specifications, and the Agentic Domain specs.
*   **Key Duty:** Designing rigorous, unambiguous specifications and enforcing architectural invariants across both domains.

### The Builder Agent
*   **Focus:** "The How".
*   **Ownership:** Implementation code, hardware abstraction layers, tests, and the Agentic DevOps tools.
...
1.  **Design:** Architect creates/refines a feature file in the appropriate domain.
